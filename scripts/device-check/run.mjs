...
/** The real brief, straight out of the live database. See its use below. */
const LIVE_BRIEF = JSON.parse(
  readFileSync(new URL('../../src/__tests__/fixtures/live-training-brief.json', import.meta.url), 'utf8'),
);
...
const ROUTES = [
  ...
  // The payload PRODUCTION serves, generated by running the real buildBrief
  // over rows copied out of the live database — including the JSONB notes
  // objects that the brief typed as strings and that threw
  // "Objects are not valid as a React child" mid-render, taking the whole
  // pt-os segment to its error boundary. Kept in its pre-fix shape on
  // purpose: it is the payload a browser sees until the API redeploys, and
  // this screen has to survive it either way.
  ['client-brief-live-payload', `/pt-os/workout-plans?client_id=${IDS.client}`, undefined,
    [[/training-brief/, 200, JSON.stringify({ data: LIVE_BRIEF })]]],
...
const AUDIT = ({ width, minTouch }) => {
  ...
  for (const el of minTouch ? document.querySelectorAll('button, a[href], input, select, [role="tab"]') : []) {
    ...
    // Measure the LAID-OUT box, not the painted one. getBoundingClientRect
    // includes ancestor transforms, and this app animates panels and toasts
    // in with `scale: 0.94 → 1`, so an element caught mid-entry measures 41
    // for a button that is genuinely 44. Two runs disagreed on the same
    // markup purely on timing. offsetWidth/offsetHeight are layout values and
    // ignore transforms, which is the honest number for "is this big enough
    // to hit" on a UI at rest. Falls back to the rect for SVG and anything
    // else with no offset box.
    const w = el.offsetWidth || r.width;
    const h = el.offsetHeight || r.height;
    if (h < minTouch - 0.5 || w < minTouch - 0.5) {
      problems.push({
        kind: 'touch-target',
        detail: `${label(el)} is ${Math.round(w)}×${Math.round(h)}`,
      });
    }
  }
