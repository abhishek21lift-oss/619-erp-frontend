...
export default function FaceEnrollModal({ clientId, clientName, open, onClose, onEnrolled }: Props) {
  ...
  const enroll = useCallback(async (samples: Float32Array[]) => {
    ...
    try {
      ...
      setState('error');
      setError(e?.message || 'Failed to save face descriptor. Please try again.');
      enrolledRef.current = false;
    }
  ...
  useEffect(() => {
    ...
    // FIX: loopKeyRef.current intentionally NOT in deps — loop manages its own lifecycle.
    // We only restart when camera or model status changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, camera.status, detection.modelStatus]);
