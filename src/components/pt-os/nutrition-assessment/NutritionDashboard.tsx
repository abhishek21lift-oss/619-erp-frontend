...
export interface NutritionScores {
  ...
  proteinAssessment: string | null;
  hydrationScore: number | null;
  digestiveHealthScore: number | null;
  supplementScore: number | null;
  nutritionScore: number | null;
  nutritionReadiness: string | null;
}

const CATEGORY_META: { key: keyof NutritionScores; label: string }[] = [
  { key: 'dietQualityScore', label: 'Diet Quality' },
  { key: 'proteinScore', label: 'Protein' },
  { key: 'hydrationScore', label: 'Hydration' },
  { key: 'digestiveHealthScore', label: 'Digestive' },
  { key: 'supplementScore', label: 'Supplement' },
];
    ...
    <div className="rounded-[24px] overflow-hidden" style={{ background: 'linear-gradient(135deg,#0f172a 0%,#1e293b 100%)', boxShadow: '0 12px 40px rgba(15,23,42,0.25)' }}>
      <div className="p-7 sm:p-10">
        <div className="flex items-start gap-4 mb-7">
          ...
          <div>
            <h2 className="text-[20px] font-[840] tracking-[-0.03em] text-white leading-none">Nutrition Dashboard</h2>
            <p className="text-[13px] text-white/40 mt-1.5">Diet quality &amp; nutrition readiness, at a glance.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div>
            <DonutChart
              ...
              centerLabel="Nutrition Score"
              hideLegend
              thin
              height={220}
            />
            {scores.nutritionReadiness && (
        ...
        <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          {scores.proteinAssessment && (
            <div className="rounded-[14px] p-4" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
              <p className="text-[10.5px] font-[700] uppercase tracking-wider text-white/40">Protein Level</p>
              <p className="mt-1 text-[16px] font-[800] text-white">{scores.proteinAssessment}</p>
            </div>
          )}
        </div>
      </div>
