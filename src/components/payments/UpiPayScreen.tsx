...
/**
 * The payment screen — where a member actually pays.
 *
 * Rendered by two routes: /member/pay/[orderId] inside the member app, and
 * /pay/[orderId] inside the studio app for an order the trainer opened at the
 * desk. The portal guard allows each account only its own app, so a member
 * sent to /pay/… was bounced straight back to their dashboard — which is why
 * the screen lives here and each app mounts it under its own shell.
 *
...
import {
  AmountBreakdown, STATUS_META, UpiStatusBadge, fmtCountdown, fmtMoneyExact, orderTermLabel,
...
export default function UpiPayScreen({ backHref, backLabel }: {
  /** Where "not found" sends the viewer: their own app's payments list. */
  backHref: string;
  backLabel: string;
}) {
  ...
  if (loadError || !detail) {
    return (
      <div className="mx-auto max-w-md py-16 text-center">
        ...
        <Button className="mt-6" onClick={() => router.push(backHref)}>
          {backLabel}
        </Button>
      </div>
    );
    ...
    <div className="mx-auto w-full max-w-[560px] pb-16 pt-2 sm:pt-6">
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <p className="mt-1 text-[13.5px]" style={{ color: 'var(--text-muted)' }}>
          {orderTermLabel(order)} · Order {order.order_no}
        </p>
...
function UtrForm({
  orderId, isBalance, previousRejection, onSubmitted, toast, className,
}: {
  orderId: string;
  isBalance: boolean;
  ...
  return (
    <Panel className={className}>
      ...
      <p className="mt-3 flex items-start gap-2 text-[11.5px]" style={{ color: 'var(--text-muted)' }}>
        ...
        {isBalance
          ? 'Your balance is updated as soon as the studio confirms the payment in its bank account.'
...
function ApprovedPanel({
  activation, isBalance, orderId, className,
}: {
  activation: NonNullable<UpiOrderDetail['activation']>;
  /** A balance payment settled what was owed; it activated no window. */
  isBalance: boolean;
  orderId: string;
  className?: string;
}) {
  return (
    <Panel className={className}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="text-[14px] font-[720]" style={{ color: 'var(--text-primary)' }}>
            {isBalance ? 'Payment received' : 'Membership active'}
          </h2>
          <p className="mt-1 text-[12.5px]" style={{ color: 'var(--text-muted)' }}>
            {activation.activated_from && activation.activated_to ? (
              <>
