...
export const Header: React.FC = () => {
  const { transport, refresh, refreshing } = useCommandCenterSnapshot(5_000);
  ...
  return (
    <div className="flex items-center gap-2.5 rounded-[16px] px-3.5 py-3 sm:gap-3 sm:px-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[11.5px]" style={{ color: 'var(--text-tertiary)' }}>{transport === 'stream' ? 'Live' : `Polling ${5_000 / 1000}s`}</p>
      </div>
      <button
        type="button"
        onClick={() => refresh()}
