...
import { Activity, Bot, Building2, CreditCard, HeartPulse, LayoutDashboard, Menu, ShieldAlert, ToggleRight, Users2, X } from 'lucide-react';
...
export default function PlatformShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-[100dvh]" style={{ background: 'var(--bg-canvas)' }}>
    <style>{`@media (min-width: 1024px) { .cc-content { --cc-sidebar-offset: ${SIDEBAR_W}px; } }
      .cc-content main .relative.mb-4.overflow-hidden.rounded-\\[20px\\] { display:none !important; }
    `}</style>
    ...
    <div style={{ paddingLeft: 'var(--cc-sidebar-offset, 0px)' }} className="cc-content">
      <header className="sticky top-0 z-20" style={{ background: 'linear-gradient(180deg, color-mix(in srgb, var(--bg-elevated) 90%, #0067E0 10%) 0%, color-mix(in srgb, var(--bg-elevated) 97%, white 3%) 100%)', borderBottom: '1px solid color-mix(in srgb, var(--brand) 14%, var(--border))', paddingTop: 'env(safe-area-inset-top, 0px)', backdropFilter: 'blur(22px) saturate(165%)', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.07), inset 0 1px 0 rgba(255,255,255,0.65)' }}>
        <div className={CONTAINER}><div className="relative flex items-center justify-between gap-3" style={{ minHeight: 62 }}>
          <div aria-hidden className="pointer-events-none absolute -left-10 -top-14 h-32 w-32 rounded-full" style={{ background: 'radial-gradient(circle, #0067E0 0%, transparent 70%)', opacity: 0.11, filter: 'blur(24px)' }} />
          <div className="relative flex min-w-0 items-center gap-2.5 sm:gap-3">
            <div className="relative flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-[12px] sm:h-10 sm:w-10 sm:rounded-[13px]" style={{ background: 'linear-gradient(145deg, #7FB4FF 0%, #0067E0 48%, #0050AD 100%)', color: '#fff', boxShadow: '0 7px 18px rgba(0,103,224,0.30), inset 0 1px 0 rgba(255,255,255,0.40)' }}><span aria-hidden className="pointer-events-none absolute inset-0" style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0.30), transparent 58%)' }} /><LayoutDashboard size={18} className="relative sm:h-[19px] sm:w-[19px]" /></div>
            <div className="min-w-0"><div className="flex min-w-0 items-center gap-2"><span className="truncate text-[15px] font-[850] tracking-[-0.025em] sm:text-[16px]" style={{ backgroundImage: 'linear-gradient(90deg, var(--text-primary) 0%, color-mix(in srgb, var(--brand) 72%, var(--text-primary)) 100%)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>Command Center</span><span className="hidden items-center gap-1.5 rounded-full px-2 py-1 text-[9px] font-[800] uppercase tracking-[0.08em] sm:inline-flex" style={{ color: 'var(--success-text)', background: 'color-mix(in srgb, var(--success) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--success) 18%, transparent)' }}><span className="relative flex h-1.5 w-1.5"><span className="absolute h-full w-full animate-ping rounded-full opacity-40" style={{ background: 'var(--success)' }} /><span className="relative h-1.5 w-1.5 rounded-full" style={{ background: 'var(--success)' }} /></span>Live</span></div><div className="mt-0.5 flex min-w-0 items-center gap-1.5 text-[10.5px] font-[650] sm:text-[11px]" style={{ color: 'var(--text-muted)' }}><span className="truncate">{activeLabel}</span><span aria-hidden style={{ color: 'var(--brand)' }}>•</span><span className="truncate">{activeTabLabel}</span></div></div>
          </div>
          <div className="relative flex shrink-0 items-center gap-2"><div className="hidden items-center gap-1.5 rounded-full px-2.5 py-1.5 text-[10px] font-[750] md:flex" style={{ background: 'color-mix(in srgb, var(--brand) 7%, var(--bg-subtle))', border: '1px solid color-mix(in srgb, var(--brand) 13%, var(--border))', color: 'var(--text-muted)' }}><Activity size={12} style={{ color: 'var(--brand)' }} /><span>Platform</span></div><div className="flex items-center rounded-full px-3 py-2 text-[10px] font-[750] sm:px-3.5" style={{ background: 'linear-gradient(180deg, var(--bg-elevated), var(--bg-subtle))', border: '1px solid var(--border)', color: 'var(--text-primary)', boxShadow: '0 4px 12px rgba(15,23,42,0.05), inset 0 1px 0 rgba(255,255,255,0.55)' }} aria-label="Current section"><span className="hidden sm:inline">{activeLabel}</span><span className="sm:hidden">{activeTabLabel}</span></div></div>
        </div></div>
      </header>
