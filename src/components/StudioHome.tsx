'use client';
// The `/` route's client half.
//
// It lives in components/ rather than in app/ for one reason: `/` is the only
// route on the origin that a search engine should index, and page-level
// `metadata` can only be exported from a SERVER component. Keeping this file
// as the page made `/` un-indexable without exception — the root layout's
// `robots: { index: false }` applied and nothing could override it.
//
// So app/(chrome)/page.tsx is now a five-line server component that carries the
// metadata and renders this. Nothing about the behaviour below changed in that
// move; it is the same component with the same branches, and a signed-in
// user's tree is identical to what it was.
