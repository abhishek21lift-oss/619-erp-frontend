    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontWeight: 700, color: '#0f172a', fontSize: 14, lineHeight: 1.3 }}>{c.name}</div>
          {(c.member_code ?? c.client_id) && (
            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2, fontFamily: 'monospace' }}>{c.member_code ?? c.client_id}</div>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {c.trainer_name && (
          <div style={{ fontSize: 11, color: '#94a3b8', textAlign: 'center' }}>👤 {c.trainer_name}</div>
        )}
      </div>
...
function PremiumEmptyState({ title, desc, accent, icon }: { title: string; desc: string; accent: string; icon: React.ReactNode }) {
  return (
    <div style={{ padding: '72px 32px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      ...
      <div>
        <p style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 6px' }}>{title}</p>
        <p style={{ fontSize: 14, color: '#94a3b8', margin: 0, maxWidth: 320 }}>{desc}</p>
      </div>
    </div>
  );
}
      ...
      <div className="seg-page" style={{ maxWidth: 1280, margin: '0 auto', padding: '24px 20px 48px' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative' }}>
            ...
            <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', margin: 0 }}>{meta.subtitle}</p>
          </div>
          <div style={{ display: 'flex', gap: 12, position: 'relative', flexWrap: 'wrap' }}>
            <div style={{ background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(10px)', borderRadius: 16, padding: '16px 20px', border: '1px solid rgba(255,255,255,0.2)', textAlign: 'center', minWidth: 90 }}>
              ...
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', marginTop: 2 }}>Members</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: '1 1 240px', minWidth: 0 }}>
            ...
            <input type="search" placeholder="Search name, phone, email…" value={search} onChange={(e) => setSearch(e.target.value)}
              style={{ width: '100%', padding: '10px 12px 10px 36px', borderRadius: 14, border: '1.5px solid #e2e8f0', fontSize: 13, color: '#0f172a', background: 'white', outline: 'none', boxSizing: 'border-box', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }} />
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginLeft: 'auto' }}>
            {!loading && <span style={{ fontSize: 13, color: '#64748b', fontWeight: 500 }}>{visible.length} member{visible.length !== 1 ? 's' : ''}</span>}
            <button onClick={fetchClients} disabled={loading} style={{ width: 36, height: 36, borderRadius: 10, border: '1.5px solid #e2e8f0', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: loading ? 'not-allowed' : 'pointer', color: '#64748b' }}>
                    ...
                    visible.map((c) => {
                      ...
                      return (
                        <tr key={c.id} style={{ borderBottom: '1px solid #f8fafc', cursor: 'pointer', transition: 'background 150ms' }} onClick={() => router.push(`/clients/${c.id}`)}>
                          <td style={{ padding: '14px 16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              ...
                              <div>
                                ...
                                {phone && <div style={{ fontSize: 11, color: '#94a3b8' }}>{phone}</div>}
                              </div>
                            </div>
                          </td>
                          <td style={{ padding: '14px 16px' }}>
                          ...
                          <td style={{ padding: '14px 16px', fontSize: 13, color: '#64748b' }}>{getPlan(c) ?? '—'}</td>
                          <td style={{ padding: '14px 16px' }}>
                            {expiry ? (
                          ...
                          <td style={{ padding: '14px 16px', fontSize: 13, color: '#64748b' }}>{c.trainer_name ?? '—'}</td>
                          <td style={{ padding: '14px 16px' }} onClick={(e) => e.stopPropagation()}>
                            <div style={{ display: 'flex', gap: 6 }}>
                              <Link href={`/clients/${c.id}`} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 12px', borderRadius: 10, border: '1.5px solid #e2e8f0', background: 'white', fontSize: 12, fontWeight: 600, color: '#475569', textDecoration: 'none' }}><Eye size={11} /> View</Link>
                              {phone && (
...
export default function MemberSegmentPage({ segment }: { segment: Segment }) {
  return <Guard><Inner segment={segment} /></Guard>;
}
