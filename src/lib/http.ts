...
//     This prevents the race condition between http.ts and Guard.tsx, and
//     ensures _redirecting is always reset cleanly on the next login.
...
//  Instead of `window.location.href = '/login'` (hard navigation that:
//    1. races with Guard.tsx's router.replace('/login')
//    2. sets _redirecting=true permanently across SPA navigations)
//  we now fire a CustomEvent. AuthProvider (auth-context.tsx) listens
//  for this event, calls logout(), and navigates via the Next.js router.
//
//  The _redirecting guard is kept to prevent duplicate events from
//  parallel in-flight requests, but is reset via the 'session-restored'
//  event fired by AuthProvider after a successful login.
// ──────────────────────────────────────────────────────────────────────
...
function handleUnauthorized(): void {
  ...
  // Skip if already on the login page — avoids redirect loop
  if (window.location.pathname === '/login') return;
  _redirecting = true;
  try { localStorage.removeItem(SESSION_USER_KEY); } catch { /* noop */ }
  // Emit event — AuthProvider handles the actual navigation
  ...
  if (!res.ok) {
    ...
    try {
      ...
      if (payload && typeof payload === 'object') {
        ...
        // Handle both flat { error: 'msg' } and nested { error: { message: 'msg' } } shapes
