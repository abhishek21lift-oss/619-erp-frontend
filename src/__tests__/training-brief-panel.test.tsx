...
//
// Not written by hand from the TypeScript types — generated by running the
// real `buildBrief` over rows copied out of the live database, which is the
// only reason this defect was found. A fixture authored from the declared type
// agrees with the type by construction and can never disagree with reality;
// this one can, and did.
//
// `pt_parq_forms.trainer_notes` and `pt_lifestyle_assessments.coach_notes` are
// JSONB OBJECTS — one field per prompt on the assessment screen — not the
// strings the brief typed them as. Handed to React as a child, an object
// throws "Objects are not valid as a React child" in the middle of the render,
// and that took the whole pt-os segment to its error boundary. An object whose
// every value is an empty string is still truthy, so it threw for every client
// whose PAR-Q had ever been opened.
import LIVE from './fixtures/live-training-brief.json';

describe('TrainingBriefPanel — the real payload', () => {
  beforeEach(() => vi.clearAllMocks());

  it('renders the brief a live client actually produces', async () => {
    render_(LIVE);
    await waitFor(() => expect(screen.getByText(/Training brief/)).toBeTruthy());
    // The parts that must survive the notes fields, not just the absence of a throw.
    expect(screen.getByText('Neck')).toBeTruthy();
    expect(screen.getByText('Anterior Pelvic Tilt')).toBeTruthy();
    expect(screen.getByText(/Approved for Exercise|Low/)).toBeTruthy();
  });

  it('never hands a non-string to a text slot', () => {
    const b = normaliseBrief(LIVE)!;
    for (const [name, value] of [
      ['readiness.notes', b.sections.readiness.notes],
      ['lifestyle.notes', b.sections.lifestyle.notes],
      ['limitations.injuries', b.sections.limitations.injuries],
      ['goal.description', b.sections.goal.description],
    ] as const) {
      expect(typeof value === 'string' || value == null, `${name} is ${typeof value}`).toBe(true);
    }
  });

  it('copies to text without throwing on the real payload', () => {
    expect(() => briefAsText(normaliseBrief(LIVE)!)).not.toThrow();
  });
});
