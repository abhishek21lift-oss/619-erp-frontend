      ...
      <div className="relative" style={{ zIndex: 1 }}>
          ...
          {flash && (
            <motion.div
              ...
              style={{
                ...
                color: 'var(--bg-white)', boxShadow: '0 8px 32px rgba(0,0,0,0.2)',
              }}
            >{flash}</motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="mb-8" style={{
          ...
          <div className="flex items-center gap-3 mb-4">
            <button onClick={() => router.back()}
              ...
              style={{ color: 'var(--text-muted)', background: 'rgba(255,255,255,0.7)', padding: '6px 12px', borderRadius: 8, border: '1px solid rgba(0,0,0,0.08)' }}
            >
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h1 style={{ fontSize: 28, fontWeight: 900, letterSpacing: '-0.04em', color: 'var(--text-primary)', lineHeight: 1 }}>Create Membership Plan</h1>
                  <p style={{ fontSize: 14, color: 'var(--text-muted)', marginTop: 4 }}>Build premium pricing plans with advanced fitness business controls</p>
                </div>
              </div>
            </div>
            {/* AI Insight Card */}
            <div className="hidden lg:flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: `linear-gradient(135deg, ${colorObj.from}15, ${colorObj.to}10)`, border: `1px solid ${colorObj.from}25` }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 500 }}>Avg quarterly plan converts 3× better than monthly</div>
              </div>
            </div>
          </div>
            ...
            {(['Membership', 'PT'] as PlanKind[]).map(k => (
              <button key={k} onClick={() => setKind(k)}
                style={{
                  ...
                  color: kind === k ? 'var(--bg-white)' : 'var(--text-muted)',
        ...
        <div className="grid lg:grid-cols-[1fr_420px] gap-8">
          ...
          <div>
              ...
              {sections.map(s => (
                <button key={s.id} onClick={() => setActiveSection(s.id)}
                  style={{
                    ...
                    color: activeSection === s.id ? 'var(--bg-white)' : 'var(--text-muted)',
                    boxShadow: activeSection === s.id ? `0 4px 12px ${colorObj.from}30` : '0 1px 3px rgba(0,0,0,0.06)',
                  }}
                >
                  {s.icon} {s.label}
                  {s.id < activeSection && <Check size={12} style={{ color: activeSection === s.id ? 'var(--bg-white)' : '#10b981' }} />}
                </button>
              ))}
            </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {/* ─── BASIC INFO ─── */}
                {activeSection === 0 && (
                  <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: '2rem', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 4px 24px rgba(0,0,0,0.04)' }}>
                    <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 24 }}>Plan Details</h2>

                    <div style={{ marginBottom: 20 }}>
                      <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Plan Name *</label>
                    ...
                    <div style={{ marginBottom: 20 }}>
                      <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Description</label>
                    ...
                    {/* Color Theme */}
                    <div style={{ marginBottom: 20 }}>
                      <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Card Color Theme</label>
                    ...
                    <div className="flex items-center justify-between" style={{ padding: '16px', borderRadius: 12, background: popular ? `${colorObj.from}10` : '#f8fafc', border: `1.5px solid ${popular ? colorObj.from + '30' : 'rgba(0,0,0,0.08)'}`, transition: 'all 0.2s' }}>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>⭐ Mark as Popular</div>
                        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Highlights this plan with a badge on the pricing page</div>
                      </div>
                      ...
                      >
                        <span style={{
                          ...
                          borderRadius: '50%', background: 'var(--bg-white)', transition: 'left 0.3s',
                    ...
                    <div className="flex justify-end mt-6">
                      <button onClick={() => setActiveSection(1)}
                        style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 24px', borderRadius: 12, fontWeight: 700, fontSize: 14, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg, ${colorObj.from}, ${colorObj.to})`, color: 'var(--bg-white)', boxShadow: `0 4px 16px ${colorObj.from}40` }}
                      >
                ...
                {activeSection === 1 && (
                  <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: '2rem', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 4px 24px rgba(0,0,0,0.04)' }}>
                    <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 24 }}>Pricing Structure</h2>

                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Base Amount (₹)</label>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Discount (₹)</label>
                    ...
                    <div style={{ marginBottom: 20 }}>
                      ...
                      />
                      <div className="flex justify-between" style={{ fontSize: 12, color: 'var(--text-disabled)', marginTop: 4 }}>
                    ...
                    <div style={{ padding: '16px 20px', borderRadius: 14, background: `linear-gradient(135deg, ${colorObj.from}10, ${colorObj.to}08)`, border: `1.5px solid ${colorObj.from}20`, marginBottom: 20 }}>
                      ...
                      <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-primary)', marginTop: 4 }}>{fmtINR(finalAmount)}</div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>{fmtINR(finalAmount)} + {fmtINR(taxAmount)} GST + {fmtINR(joiningFee)} joining fee</div>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Joining Fee (₹)</label>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>GST (%)</label>
                    ...
                    <div className="flex justify-between mt-6">
                      <button onClick={() => setActiveSection(0)}
                        style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 20px', borderRadius: 12, fontWeight: 700, fontSize: 14, border: '1.5px solid rgba(0,0,0,0.1)', cursor: 'pointer', background: 'var(--bg-white)', color: 'var(--text-muted)' }}
                      >
                ...
                {activeSection === 2 && (
                  <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: '2rem', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 4px 24px rgba(0,0,0,0.04)' }}>
                    <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 24 }}>Plan Duration</h2>
                      ...
                      {DURATIONS.map(d => (
                        <button key={d} onClick={() => setDuration(d)}
                          style={{
                            ...
                            color: duration === d ? 'var(--bg-white)' : 'var(--text-muted)',
                    ...
                    {kind === 'PT' && (
                      <div style={{ marginBottom: 20, padding: '20px', borderRadius: 14, background: `${colorObj.from}08`, border: `1.5px solid ${colorObj.from}20` }}>
                          ...
                          {[2, 3, 4, 5, 6].map(n => (
                            <button key={n} onClick={() => setSessionsPerWeek(n)}
                              style={{
                                ...
                                background: sessionsPerWeek === n ? `linear-gradient(135deg, ${colorObj.from}, ${colorObj.to})` : 'var(--bg-white)',
                                color: sessionsPerWeek === n ? 'var(--bg-white)' : 'var(--text-muted)',
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8 }}>{sessionsPerWeek}x per week · {sessionsPerWeek * 4} sessions/month</div>
                      </div>
                    )}
                    ...
                    {finalAmount > 0 && (
                      <div style={{ padding: '20px', borderRadius: 14, background: '#f8fafc', border: '1px solid rgba(0,0,0,0.06)' }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Revenue Projections</div>
                        {[10, 25, 50].map(members => (
                          <div key={members} className="flex justify-between items-center" style={{ padding: '8px 0', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
                            <span style={{ fontSize: 14, color: 'var(--text-muted)' }}>{members} members</span>
                            <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>{fmtINR(members * finalAmount)}<span style={{ fontSize: 12, color: 'var(--text-disabled)' }}>/mo</span></span>
                          </div>
                        ))}
                      </div>
                    )}
                ...
                {activeSection === 3 && (
                  <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: '2rem', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 4px 24px rgba(0,0,0,0.04)' }}>
                    <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 8 }}>Plan Benefits</h2>
                    <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>Select what's included in this plan</p>
                        ...
                        return (
                          <button key={label}
                            ...
                            style={{
                              ...
                              color: selected ? 'var(--bg-white)' : 'var(--text-muted)',
                    ...
                    <div className="flex gap-2 mb-4">
                      ...
                      <button onClick={() => addBenefit(newBenefit)}
                        style={{ width: 40, height: 40, borderRadius: 10, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg, ${colorObj.from}, ${colorObj.to})`, color: 'var(--bg-white)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                      ><Plus size={16} /></button>
                    </div>
                    ...
                    {benefits.length > 0 && (
                      <div style={{ marginBottom: 20 }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>Selected ({benefits.length})</div>
                    ...
                    <div className="flex justify-between mt-6">
                      ...
                      <div className="flex gap-3">
                        <button onClick={() => handleSave(false)} disabled={saving}
                          style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 20px', borderRadius: 12, fontWeight: 700, fontSize: 14, border: '1.5px solid rgba(0,0,0,0.1)', cursor: saving ? 'not-allowed' : 'pointer', background: 'var(--bg-white)', color: 'var(--text-muted)', opacity: saving ? 0.6 : 1 }}
                        >
                          <Save size={15} /> Save Draft
                        </button>
                        <button onClick={() => handleSave(true)} disabled={saving}
                          style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 24px', borderRadius: 12, fontWeight: 700, fontSize: 14, border: 'none', cursor: saving ? 'not-allowed' : 'pointer', background: saving ? 'var(--text-disabled)' : `linear-gradient(135deg, ${colorObj.from}, ${colorObj.to})`, color: 'var(--bg-white)', boxShadow: saving ? 'none' : `0 4px 16px ${colorObj.from}40`, opacity: saving ? 0.8 : 1 }}
                        >
          ...
          <div className="lg:sticky lg:top-24 h-fit">
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-disabled)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Live Preview</div>
            ...
            <div style={{
              ...
              <div style={{ padding: '28px 28px 24px', background: `linear-gradient(135deg, ${colorObj.from}, ${colorObj.to})`, position: 'relative', overflow: 'hidden' }}>
                ...
                {popular && (
                  <div style={{ position: 'absolute', top: 20, right: 20, display: 'flex', alignItems: 'center', gap: 4, padding: '4px 12px', borderRadius: 999, background: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(10px)', fontSize: 11, fontWeight: 700, color: 'var(--bg-white)' }}>
                    <Star size={11} fill="#fff" /> POPULAR
                  </div>
                )}
                <div style={{ fontSize: 26, fontWeight: 900, color: 'var(--bg-white)', letterSpacing: '-0.03em', lineHeight: 1.1 }}>{name || 'Plan Name'}</div>
                <div style={{ fontSize: 13, color: 'var(--text-primary)', marginTop: 6 }}>{description || 'Your plan description will appear here'}</div>
                <div className="flex items-baseline gap-2" style={{ marginTop: 16 }}>
                  <span style={{ fontSize: 36, fontWeight: 900, color: 'var(--bg-white)', letterSpacing: '-0.04em' }}>{fmtINR(finalAmount)}</span>
                ...
                {duration !== 'Monthly' && finalAmount > 0 && (
                  <div style={{ marginTop: 6, fontSize: 12, color: 'var(--text-primary)' }}>{fmtINR(Math.round(monthlyRate))}/month equivalent</div>
                )}
              </div>

              {/* Card Body */}
              <div style={{ padding: '20px 28px 28px', background: 'var(--bg-white)' }}>
                {kind === 'PT' && (
                  <div className="flex gap-2 mb-4">
                ...
                {benefits.length > 0 && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--text-disabled)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>INCLUDES</div>
                    {benefits.slice(0, 6).map(b => (
                      <div key={b} className="flex items-center gap-2" style={{ padding: '5px 0', fontSize: 13, color: 'var(--text-muted)' }}>
                        <Check size={13} style={{ color: colorObj.from, flexShrink: 0 }} />
                        {b}
                      </div>
                    ))}
                    {benefits.length > 6 && <div style={{ fontSize: 12, color: 'var(--text-disabled)', marginTop: 4 }}>+{benefits.length - 6} more benefits</div>}
                  </div>
                )}
                ...
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: 16 }}>
                  {[
                    ...
                    <div key={row.label} className="flex justify-between" style={{ fontSize: 13, color: 'var(--text-disabled)', marginBottom: 6 }}>
                      <span>{row.label}</span><span>{row.value}</span>
                    </div>
                  ))}
                  <div className="flex justify-between" style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-primary)', marginTop: 8, paddingTop: 8, borderTop: '1px solid #f1f5f9' }}>
            ...
            {finalAmount > 0 && (
              <div style={{ marginTop: 16, padding: '20px', borderRadius: 16, background: 'var(--bg-card)', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 4px 16px rgba(0,0,0,0.04)' }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-disabled)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Revenue Projections</div>
                {[10, 25, 50].map(m => (
                  <div key={m} className="flex justify-between items-center" style={{ padding: '8px 0', borderBottom: '1px solid #f1f5f9' }}>
                    <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{m} members</span>
                    <span style={{ fontSize: 14, fontWeight: 800, color: 'var(--text-primary)' }}>{fmtINR(m * finalAmount)}<span style={{ fontSize: 11, color: 'var(--text-disabled)' }}>/mo</span></span>
                  </div>
                ))}
              </div>
            )}
