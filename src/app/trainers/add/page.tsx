...
import {
  ...
  Dumbbell, Star, Zap, Calendar, Camera,
} from 'lucide-react';
...
const SPECIALIZATIONS = [
  'Strength Training', 'HIIT', 'Yoga', 'Pilates', 'Cardio',
  'Powerlifting', 'CrossFit', 'Functional Training', 'Rehabilitation',
  'Nutrition', 'Weight Loss', 'Muscle Gain', 'Endurance', 'Flexibility',
];

const CERTIFICATIONS = [
  'K11 Fitness', 'ACE Certified', 'NASM CPT', 'ISSA', 'ACSM',
  'CrossFit L1', 'Yoga RYT 200', 'Sports Nutrition', 'First Aid CPR',
];
      ...
      {options.map((o) => {
        ...
        return (
          <button
            ...
            style={{
              background: on ? color : 'rgba(15,23,42,0.04)',
...
// ── ToggleSwitch ─────────────────────────────────────────────────────
function ToggleSwitch({ label, sublabel, defaultOn }: { label: string; sublabel?: string; defaultOn?: boolean }) {
  const [on, setOn] = useState(defaultOn ?? false);
  return (
    <div className="flex items-center justify-between rounded-[14px] px-4 py-3.5"
      style={{ background: 'rgba(248,250,252,0.8)', border: '1px solid rgba(15,23,42,0.07)' }}>
      <div>
      ...
      <button
        ...
        style={{ background: on ? '#6366f1' : 'rgba(15,23,42,0.12)' }}
        aria-checked={on}
        role="switch"
      >
        <span
          className="absolute top-[3px] h-[18px] w-[18px] rounded-full bg-white shadow-sm transition-all duration-300"
    ...
    <div>
      ...
      <div
        ...
        onDrop={(e) => {
          ...
          const f = Array.from(e.dataTransfer.files).map((f) => f.name);
          setFiles((p) => [...p, ...f]);
        }}
        className="flex cursor-pointer flex-col items-center gap-2 rounded-[16px] px-6 py-8 text-center transition-all"
        style={{
          border: dragging ? '2px dashed #6366f1' : '2px dashed rgba(15,23,42,0.12)',
          background: dragging ? 'rgba(99,102,241,0.04)' : 'rgba(248,250,252,0.6)',
          boxShadow: dragging ? '0 0 0 4px rgba(99,102,241,0.06)' : 'none',
        }}
      >
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {files.map((f, i) => (
            <span key={i} className="flex items-center gap-1 rounded-full px-3 py-1 text-[11px] font-[600]"
              ...
              <FileText size={10} />{f}
              <button onClick={() => setFiles((p) => p.filter((_, j) => j !== i))} className="ml-0.5">
                <X size={9} />
              </button>
            </span>
          ))}
  ...
  const stepContent: Record<number, React.ReactNode> = {
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-center pb-2">
          <button type="button" onClick={() => photoRef.current?.click()}
            ...
            {photoPreview
              ? <img src={photoPreview} alt="Coach" className="h-full w-full object-cover" />
              : <Camera size={24} style={{ color: 'rgba(99,102,241,0.5)' }} />}
            <span className="absolute inset-0 flex items-center justify-center rounded-[22px] bg-black/20 opacity-0 transition-opacity group-hover:opacity-100">
        ...
        <Select label="Experience Level" options={['Beginner (0-1 yr)', 'Intermediate (1-3 yrs)', 'Advanced (3-5 yrs)', 'Expert (5+ yrs)']} required />
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
    ...
    2: (
      <div className="space-y-4">
        ...
        <Input label="Emergency Contact Name" required />
        <Input label="Emergency Contact Phone" type="tel" required />
    ...
    4: (
      <div className="space-y-4">
        <Select label="Salary Type" options={['Fixed Monthly', 'Per Session', 'Revenue Share', 'Hybrid']} required />
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Input label="Base Salary (₹)" type="number" required />
    ...
    5: (
      <div className="space-y-5">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Input label="Shift Start Time" type="time" required />
          <Input label="Shift End Time" type="time" required />
        </div>
        <div>
            ...
            {DAYS.map((d) => {
              ...
              return (
                <button key={d} type="button"
                  ...
                  style={{
                    background: on ? '#10b981' : 'rgba(15,23,42,0.04)',
                    color: on ? '#fff' : 'rgb(100,116,139)',
                    border: on ? '1.5px solid #10b981' : '1.5px solid transparent',
    ...
    6: (
      <div className="space-y-4">
        ...
        <Select label="Studio Access Level" options={['Full Access', 'Floor Only', 'Limited Hours', 'Custom']} required />
      ...
      <div className="sticky top-0 z-30 border-b" style={{
        ...
        <div className="mx-auto flex max-w-3xl items-center gap-4 px-5 py-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-[12px]" style={{ color: 'rgb(148,163,184)' }}>Step {step} of {STEPS.length} — {current.title}</p>
          </div>
        ...
        {/* Progress bar */}
        <div className="h-[2px] w-full" style={{ background: 'rgba(15,23,42,0.06)' }}>
