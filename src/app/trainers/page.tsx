...
import {
  ...
  Mail, Phone, ChevronRight, BarChart3, Edit3, UserCheck,
  DollarSign, Target, Sparkles, ShieldCheck, Clock,
  Crown, Medal, ArrowUpRight, Zap, Activity,
} from 'lucide-react';
      ...
      <div className={cn(
          ...
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              <Link href={`/trainers/${trainer.id}`} className="hover:opacity-80 transition-opacity">
                <h3 className="text-[15px] font-bold text-[#0B0B0F] truncate">{trainer.name}</h3>
              </Link>
              <div className="flex items-center gap-1.5 mt-0.5">
                <Award size={11} className="text-[#8B5CF6] shrink-0" />
                <span className="text-[11px] text-[#4A4E57] truncate">{specialty}</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-2 mt-4">
          <div className="rounded-[12px] bg-[rgba(37,99,235,0.05)] p-2.5 text-center ring-1 ring-[rgba(37,99,235,0.08)]">
            ...
            <p className="text-[16px] font-extrabold text-[#0B0B0F] mt-0.5">{trainer.active_clients ?? 0}</p>
          </div>
          <div className="rounded-[12px] bg-[rgba(16,185,129,0.05)] p-2.5 text-center ring-1 ring-[rgba(16,185,129,0.08)]">
            ...
            <p className="text-[16px] font-extrabold text-[#0B0B0F] mt-0.5">{fmtINR(trainer.month_revenue)}</p>
          </div>
          <div className="rounded-[12px] bg-[rgba(139,92,246,0.05)] p-2.5 text-center ring-1 ring-[rgba(139,92,246,0.08)]">
            ...
            <p className="text-[16px] font-extrabold text-[#0B0B0F] mt-0.5">{trainer.total_clients ?? 0}</p>
          </div>
        </div>

        {/* Contact + Actions */}
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-[rgba(0,0,0,0.04)]">
          <div className="flex items-center gap-2 text-[11px] text-[#4A4E57]">
            {trainer.mobile && (
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Link
              ...
              className="flex items-center gap-1 rounded-[10px] bg-[rgba(0,0,0,0.03)] px-3 py-1.5 text-[11px] font-bold text-[#4A4E57] transition-all hover:bg-[rgba(0,0,0,0.06)]"
            >
              <Edit3 size={11} />
            </Link>
          </div>
    ...
    >
      <td className="py-3 px-4 w-12">
        {rankStyle ? (
          ...
          <span className="text-[12px] font-bold text-[#4A4E57]">#{rank + 1}</span>
        )}
      </td>
      <td className="py-3 px-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-[13px] font-semibold text-[#0B0B0F]">{trainer.name}</p>
            <p className="text-[10px] text-[#4A4E57]">{trainer.specialization || trainer.role || 'Trainer'}</p>
          </div>
        </div>
      </td>
      <td className="py-3 px-4 text-center">
        <span className="text-[13px] font-bold text-[#0B0B0F]">{trainer.active_clients ?? 0}</span>
      </td>
      <td className="py-3 px-4 text-right">
        <span className="text-[13px] font-bold text-[#0B0B0F]">{fmtINR(revenue)}</span>
      </td>
      <td className="py-3 px-4 text-right">
    ...
    <div className="flex items-end justify-between gap-2 h-32 mt-4">
      {data.map((d, i) => (
        <div key={d.month} className="flex-1 flex flex-col items-center gap-1.5">
          ...
          <span className="text-[9px] font-semibold text-[#4A4E57]">{d.month}</span>
        </div>
      ))}
    </div>
  );
        ...
        <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
            ...
            <GlassPanel className="p-4">
              <div className="flex flex-wrap items-center gap-3">
                <div className="relative flex-1 min-w-[200px] max-w-sm">
                  <Search size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#4A4E57]" strokeWidth={1.5} />
                  <input
                    value={search}
                    onChange={e => setSearch(e.target.value)}
                    placeholder="Search trainers by name, email, or specialization..."
                    className="w-full h-10 rounded-[12px] border border-[rgba(0,0,0,0.06)] bg-white pl-9 pr-3 text-[13px] text-[#0B0B0F] placeholder-[#9CA3AF] outline-none transition-all focus:border-[#2563EB]/30 focus:shadow-[0_0_0_3px_rgba(37,99,235,0.08)]"
                  />
                </div>
                <span className="text-[12px] font-medium text-[#4A4E57] ml-auto">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8">
                ...
                <GlassPanel className="p-5 lg:col-span-2">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#4A4E57]">Analytics</p>
                      <h3 className="text-[15px] font-bold text-[#0B0B0F] mt-0.5">Revenue Trend</h3>
                    </div>
                    <div className="flex items-center gap-1.5 text-[11px] font-medium text-[#4A4E57]">
                  ...
                  <div className="flex items-center gap-4 mt-4 pt-3 border-t border-[rgba(0,0,0,0.04)]">
                    <div className="flex items-center gap-1.5 text-[11px] text-[#4A4E57]">
                      <div className="h-2.5 w-2.5 rounded-[4px] bg-[#2563EB]" />
                      Monthly Revenue
                    </div>
                    <span className="text-[11px] font-bold text-[#0B0B0F] ml-auto">
                ...
                <GlassPanel className="p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#4A4E57]">Insights</p>
                      <h3 className="text-[15px] font-bold text-[#0B0B0F] mt-0.5">Team Summary</h3>
                    </div>
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={item.label} className="flex items-center justify-between rounded-[12px] bg-[rgba(0,0,0,0.02)] px-3.5 py-2.5">
                        <span className="text-[11px] text-[#4A4E57]">{item.label}</span>
              ...
              {sortedByRevenue.length > 0 && (
                <GlassPanel className="overflow-hidden">
                  <div className="flex items-center justify-between p-5 pb-3">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#4A4E57]">Rankings</p>
                      <h3 className="text-[15px] font-bold text-[#0B0B0F] mt-0.5">Performance Leaderboard</h3>
                    </div>
                      ...
                      <thead>
                        <tr className="border-t border-b border-[rgba(0,0,0,0.04)]">
                          {['Rank', 'Trainer', 'Clients', 'Revenue', 'Score'].map(h => (
                            <th key={h} className="py-2.5 px-4 text-[10px] font-bold uppercase tracking-[0.06em] text-[#4A4E57]">
                              {h}
                            </th>
                          ))}
                        </tr>
          ...
          {!loading && !error && filtered.length === 0 && (
            ...
            >
              ...
              <h3 className="text-[18px] font-bold text-[#0B0B0F] mb-1">
                {search ? 'No trainers match your search' : 'No trainers found'}
              </h3>
              <p className="text-[13px] text-[#4A4E57] mb-6 text-center max-w-sm">
                {search
