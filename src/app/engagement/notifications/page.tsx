...
function NContent() {
  ...
  useEffect(()=>{
    api.clients.list({status:'active'}).then((d:any)=>setMemberCount(Array.isArray(d)?d.length:0)).catch((err)=>console.error('[notifications] failed to fetch member count', err));
  },[]);
