...
const KPIS = [
  { label:'Total Members', key:'members', color:'#8b5cf6', icon:<Users size={18}/>, bg:'linear-gradient(135deg, rgba(139,92,246,0.12), rgba(109,40,217,0.06))' },
  { label:'Selected', key:'selected', color:'#a855f7', icon:<CheckCircle2 size={18}/>, bg:'linear-gradient(135deg, rgba(168,85,247,0.12), rgba(88,28,135,0.06))' },
  { label:'Sent Today', key:'sent', color:'#6d28d9', icon:<MessageCircle size={18}/>, bg:'linear-gradient(135deg, rgba(109,40,217,0.12), rgba(139,92,246,0.06))' },
];
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 380px', gap:20 }}>
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ padding:'16px 22px', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', gap:10, alignItems:'center', background:'linear-gradient(90deg, rgba(139,92,246,0.08), rgba(109,40,217,0.04))' }}>
              <div style={{ flex:1, display:'flex', alignItems:'center', gap:8, background:'rgba(255,255,255,0.05)', borderRadius:10, padding:'8px 14px', border:'1px solid rgba(255,255,255,0.08)' }}>
                <Search size={14} color="rgba(255,255,255,0.35)" />
                <input placeholder="Search members…" value={search} onChange={e=>setSearch(e.target.value)}
                  style={{ flex:1, border:'none', background:'transparent', outline:'none', fontSize:13, fontWeight:500, color:'rgba(255,255,255,0.85)' }} />
                {search&&<button onClick={()=>setSearch('')} style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,0.35)', padding:0 }}><X size={13}/></button>}
              </div>
              <button onClick={selectAll} style={{ fontSize:12, fontWeight:600, color:'#a855f7', background:'rgba(168,85,247,0.12)', border:'none', borderRadius:8, padding:'6px 14px', cursor:'pointer' }}>All</button>
              <button onClick={clearAll} style={{ fontSize:12, fontWeight:600, color:'rgba(255,255,255,0.4)', background:'rgba(255,255,255,0.05)', border:'none', borderRadius:8, padding:'6px 14px', cursor:'pointer' }}>None</button>
            </div>
            <div style={{ maxHeight:520, overflowY:'auto' }}>
              {loading?<div style={{ padding:40, textAlign:'center', color:'rgba(255,255,255,0.35)', fontSize:14 }}>Loading members…</div>:
              filtered.map(m=>(
                <div key={m.id}
                  ...
                  style={{ display:'flex', alignItems:'center', gap:12, padding:'13px 22px', borderBottom:'1px solid rgba(255,255,255,0.04)', cursor:'pointer', background:selected.includes(m.id)?'rgba(139,92,246,0.1)':'transparent', transition:'all 200ms' }}
                  onMouseEnter={e=>{if(!selected.includes(m.id))e.currentTarget.style.background='rgba(255,255,255,0.03)';}}
                  ...
                  <div style={{ width:38, height:38, borderRadius:'50%', background:'linear-gradient(135deg, rgba(139,92,246,0.2), rgba(109,40,217,0.1))', color:'#a855f7', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:12, flexShrink:0, border:'1px solid rgba(139,92,246,0.15)' }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontWeight:700, fontSize:13, color:'rgba(255,255,255,0.9)' }}>{m.name}</div>
                    <div style={{ fontSize:11, color:'rgba(255,255,255,0.35)', display:'flex', gap:14 }}>
                  ...
                  <button onClick={e=>{e.stopPropagation();openWhatsApp(m);}} disabled={!m.mobile}
                    style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, fontWeight:700, padding:'6px 16px', borderRadius:10, background:m.mobile?'linear-gradient(135deg, #8b5cf6, #6d28d9)':'rgba(255,255,255,0.04)', color:m.mobile?'#fff':'rgba(255,255,255,0.25)', border:'none', cursor:m.mobile?'pointer':'not-allowed', boxShadow:m.mobile?'0 4px 12px rgba(139,92,246,0.3)':'none' }}>
                    <MessageCircle size={12}/> Send
                  </button>
                </div>
              ))}
              {!loading && filtered.length===0 && <div style={{ padding:40, textAlign:'center', color:'rgba(255,255,255,0.35)', fontSize:14 }}>No members found</div>}
            </div>
            {selected.length>0&&(
              <div style={{ padding:'14px 22px', borderTop:'1px solid rgba(255,255,255,0.06)', display:'flex', justifyContent:'space-between', alignItems:'center', background:'linear-gradient(90deg, rgba(139,92,246,0.08), rgba(109,40,217,0.04))' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay:0.1 }}
              style={{ borderRadius:20, background:'rgba(255,255,255,0.06)', backdropFilter:'blur(20px)', border:'1px solid rgba(255,255,255,0.1)', boxShadow:'0 8px 32px rgba(0,0,0,0.12)', padding:22 }}>
              <h3 style={{ margin:'0 0 16px', fontSize:14, fontWeight:700, color:'rgba(255,255,255,0.9)', display:'flex', alignItems:'center', gap:8 }}>
                <MessageCircle size={15} color="#a855f7"/> Message Template
              </h3>
              <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:14 }}>
                {TEMPLATES.map(t=>(
                  <button key={t.id} onClick={()=>setTemplate(t)}
                    style={{ padding:'10px 16px', borderRadius:10, border:`1px solid ${template.id===t.id?'rgba(139,92,246,0.4)':'rgba(255,255,255,0.07)'}`, background:template.id===t.id?'linear-gradient(135deg, rgba(139,92,246,0.12), rgba(109,40,217,0.06))':'transparent', textAlign:'left', cursor:'pointer', fontSize:13, fontWeight:template.id===t.id?700:500, color:template.id===t.id?'#a855f7':'rgba(255,255,255,0.6)', transition:'all 150ms' }}>
                    {t.name}
                  </button>
                ))}
              </div>
              {template.id==='custom'?(
                ...
                <div style={{ background:'rgba(255,255,255,0.03)', borderRadius:10, padding:14, fontSize:13, color:'rgba(255,255,255,0.5)', lineHeight:1.7, whiteSpace:'pre-wrap' }}>{template.body}</div>
              )}
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay:0.15 }}
              style={{ borderRadius:20, background:'linear-gradient(135deg, rgba(34,197,94,0.08), rgba(34,197,94,0.02))', border:'1px solid rgba(34,197,94,0.15)', backdropFilter:'blur(20px)', padding:20 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8, color:'#4ade80', fontWeight:700, fontSize:13 }}><ExternalLink size={14}/>Opens WhatsApp Web</div>
              <p style={{ margin:0, fontSize:12, color:'rgba(255,255,255,0.4)', lineHeight:1.6 }}>Clicking Send opens WhatsApp Web or App with a pre-filled message. No API key needed — direct member communication.</p>
            </motion.div>
          </div>
        </div>
      </div>
