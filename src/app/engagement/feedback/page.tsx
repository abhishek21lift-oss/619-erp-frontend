          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {CATS.map(c => (
              <button key={c} onClick={() => setCat(c)}
                style={{ padding: '7px 16px', borderRadius: 8, border: `1px solid ${cat === c ? '#8b5cf6' : '#e5e7eb'}`, fontSize: 12, fontWeight: 600, cursor: 'pointer',
                  background: cat === c ? 'linear-gradient(135deg, #8b5cf6, #6d28d9)' : '#ffffff', color: cat === c ? '#fff' : '#374151' }}>{c}</button>
            ))}
          </div>
                        ...
                        {f.status !== 'resolved' && (
                          <>
                            ...
                            <button onClick={() => handleResolve(f.id)} disabled={resolving === f.id}
                              style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 600, padding: '5px 14px', borderRadius: 8, border: '1px solid rgba(22,163,74,0.25)', background: 'rgba(22,163,74,0.08)', color: '#16a34a', cursor: 'pointer' }}>
