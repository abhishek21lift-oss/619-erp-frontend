        ...
        <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <m.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}
              className="rounded-[20px] p-6" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-xs)' }}>
              <h2 className="text-[18px] font-[760] mb-5" style={{ color: 'var(--text-primary)' }}>Record Assessment</h2>
            ...
            <m.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
              className="rounded-[20px] p-6" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-xs)' }}>
              <h2 className="text-[18px] font-[760] mb-5" style={{ color: 'var(--text-primary)' }}>Assessment History</h2>
              ...
              {!assessments.loading && (!assessments.data || (assessments.data as unknown[]).length === 0) && (
                <p className="text-center py-8 text-sm" style={{ color: 'var(--text-disabled)' }}>No assessments recorded yet.</p>
              )}
              <div className="space-y-3 max-h-[500px] overflow-y-auto">
                {(assessments.data as any[] || []).map((a: any) => (
                  <div key={a.id} className="rounded-[14px] p-4" style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border)' }}>
                    <div className="flex justify-between mb-2">
                      <span className="text-[12px] font-semibold" style={{ color: 'var(--text-muted)' }}>{a.assessment_date}</span>
                    ...
                    <div className="grid grid-cols-3 gap-1 text-[11px]">
                      {a.weight && <div><span style={{ color: 'var(--text-muted)' }}>Wt: </span><span className="font-semibold" style={{ color: 'var(--text-primary)' }}>{a.weight}kg</span></div>}
                      {a.body_fat_pct && <div><span style={{ color: 'var(--text-muted)' }}>BF: </span><span className="font-semibold" style={{ color: 'var(--text-primary)' }}>{a.body_fat_pct}%</span></div>}
                      {a.bmi && <div><span style={{ color: 'var(--text-muted)' }}>BMI: </span><span className="font-semibold" style={{ color: 'var(--text-primary)' }}>{a.bmi}</span></div>}
                    </div>
                  </div>
                ))}
              </div>
