        ...
        <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <m.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
              className="rounded-[20px] p-6" style={{ background: '#fff', border: '1px solid #e5e7eb' }}>
              <h2 className="text-[18px] font-[760] mb-5" style={{ color: '#111827' }}>Log Check-In</h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <input type="number" placeholder="Adherence % (0-100)" value={adherencePct} onChange={e => setAdherencePct(e.target.value)}
                  ...
                  style={{ background: '#fff', border: '1px solid #d1d5db', color: '#111827' }} />
                <div>
                  <p className="text-[11px] font-semibold mb-2" style={{ color: '#6b7280' }}>Mood</p>
            ...
            <m.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
              className="rounded-[20px] p-6" style={{ background: '#fff', border: '1px solid #e5e7eb' }}>
              <h2 className="text-[18px] font-[760] mb-5" style={{ color: '#111827' }}>Recent Check-Ins</h2>
              <div className="space-y-3 max-h-[500px] overflow-y-auto">
                {(checkins.data as any[] || []).map((c: any) => (
                  <div key={c.id} className="rounded-[14px] p-4" style={{ background: '#F9FAFB', border: '1px solid #f3f4f6' }}>
                    <div className="flex justify-between mb-2">
                      <span className="text-[12px] font-semibold" style={{ color: '#6b7280' }}>Week of {c.week_start_date}</span>
                      {c.adherence_pct && (
                    ...
                    <div className="grid grid-cols-3 gap-2 text-[11px]">
                      {c.weight && <div><span style={{ color: '#6b7280' }}>Wt: </span><span className="font-semibold">{c.weight}kg</span></div>}
                      {c.workout_count > 0 && <div><span style={{ color: '#6b7280' }}>WOs: </span><span className="font-semibold">{c.workout_count}</span></div>}
                      {c.mood && <div><span style={{ color: '#6b7280' }}>Mood: </span><span className="font-semibold capitalize">{c.mood}</span></div>}
                    </div>
                  </div>
                ))}
                {(!checkins.data || (checkins.data as any[]).length === 0) && (
                  <p className="text-center py-8 text-sm" style={{ color: '#6b7280' }}>No check-ins yet.</p>
                )}
              </div>
            </m.div>
          </div>
