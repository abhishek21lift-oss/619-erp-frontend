...
import {
  Camera, Plus, Loader2, Trash2, Search, Users, AlertCircle, ChevronDown,
  X, Maximize2, RefreshCw, GitCompare,
} from 'lucide-react';
    ...
    <div className="mx-auto w-full max-w-3xl px-4 py-6 sm:px-6 sm:py-8">
      <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <p className="mt-3 max-w-xl text-[14px]" style={{ color: 'var(--text-muted)' }}>
          Select a client to capture or review their transformation photos.
        </p>
      </m.div>
        ...
        {!loading && !loadError && (
          <div className="flex flex-wrap gap-2 max-h-[360px] overflow-y-auto">
            {filtered.map((c) => (
              <button
                key={c.id}
                onClick={() => router.push(`/pt-os/progress-photos?client_id=${c.id}`)}
