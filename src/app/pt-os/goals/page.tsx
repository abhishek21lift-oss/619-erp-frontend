        ...
        <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <span className="text-[11px] font-[650] uppercase tracking-[0.08em]" style={{ color: '#a78bfa' }}>PT System</span>
              </div>
              <h1 className="text-[32px] sm:text-[40px] font-[860] tracking-[-0.03em] leading-tight" style={{ color: '#ffffff' }}>
                Goal Setting
              </h1>
              <p className="mt-3 max-w-xl text-[14px] sm:text-[15px]" style={{ color: 'rgba(255,255,255,0.65)' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}
              className="rounded-[20px] p-6" style={{ background: 'rgba(255,255,255,0.75)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.95)', boxShadow: '0 2px 20px rgba(15,23,42,0.06)' }}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {GOAL_TYPES.map(g => (
                    <button key={g.value} type="button" onClick={() => setGoalType(g.value)}
                      ...
                      style={{
                        background: goalType === g.value ? `${g.color}15` : 'rgba(0,0,0,0.03)',
                        border: `1.5px solid ${goalType === g.value ? g.color : 'transparent'}`,
                        color: goalType === g.value ? g.color : 'rgb(100,116,139)',
                      }}>
                      {g.icon} {g.label}
                    </button>
                  ))}
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
              ...
              {!goals.loading && (!goals.data || (goals.data as unknown[]).length === 0) && (
                <p className="text-center py-8 text-sm" style={{ color: 'rgb(148,163,184)' }}>No goals set yet. Select a client and create a goal.</p>
              )}
              <div className="space-y-3">
                {(goals.data as unknown[] || []).map((g: any) => (
                  <div key={g.id} className="rounded-[14px] p-4" style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.04)' }}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[13px] font-[700]" style={{ color: 'rgb(15,23,42)' }}>{g.goal_type?.replace(/_/g, ' ')}</span>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-[12px]">
                      {g.target_weight && <div><span style={{ color: 'rgb(148,163,184)' }}>Target: </span><span className="font-semibold" style={{ color: 'rgb(15,23,42)' }}>{g.target_weight} kg</span></div>}
                      {g.target_body_fat && <div><span style={{ color: 'rgb(148,163,184)' }}>Body Fat: </span><span className="font-semibold" style={{ color: 'rgb(15,23,42)' }}>{g.target_body_fat}%</span></div>}
                      {g.target_date && <div><span style={{ color: 'rgb(148,163,184)' }}>By: </span><span className="font-semibold" style={{ color: 'rgb(15,23,42)' }}>{g.target_date}</span></div>}
                    </div>
                  </div>
                ))}
              </div>
