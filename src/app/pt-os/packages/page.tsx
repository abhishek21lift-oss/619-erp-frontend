        ...
        <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
          ...
          {showForm && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
              className="rounded-[20px] p-6 mb-6" style={{ background: 'rgba(255,255,255,0.75)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.95)' }}>
            ...
            {(packages.data as any[] || []).map((pkg: any, i: number) => (
              <motion.div key={pkg.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                className="rounded-[20px] p-5 relative overflow-hidden" style={{
                  ...
                  border: `1px solid ${pkg.is_active ? 'rgba(255,255,255,0.95)' : 'rgba(239,68,68,0.2)'}`,
