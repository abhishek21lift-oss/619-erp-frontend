...
function NewClientForm() {
  ...
  const inputClass = "w-full h-11 px-3.5 rounded-xl border border-[rgba(0,0,0,0.04)] bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] text-[14px] text-[#1d1d1f] dark:text-[var(--text-primary)] transition-all duration-150 outline-none focus:border-[#dc2626]/30 focus:bg-white dark:bg-[#1E1F24] focus:shadow-[0_0_0_3px_rgba(220,38,38,0.06)] placeholder:text-[#86868b] dark:text-[var(--text-muted)]/60";
  const labelClass = "block text-[12px] font-semibold text-[#86868b] dark:text-[var(--text-muted)] mb-1.5 tracking-[0.01em]";

  return (
    <AppShell>
      <div className="min-h-screen bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] pb-24">
        <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6 sm:py-8">
          {/* Hero */}
          <div className="mb-6">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <div className="inline-flex items-center gap-1.5 rounded-full bg-white dark:bg-[#1E1F24] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.06em] text-[#86868b] dark:text-[var(--text-muted)] shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#34d399]" />
                  Member Onboarding
                </div>
                <h1 className="mt-3 text-[28px] sm:text-[34px] font-bold tracking-[-0.03em] text-[#1d1d1f] dark:text-[var(--text-primary)] leading-[1.1]">
                  {isEditMode ? 'Edit Member Profile' : <>Create a <span className="text-[#dc2626]">Transformation Journey</span></>}
                </h1>
                <p className="mt-2 max-w-[56ch] text-[14px] leading-relaxed text-[#86868b] dark:text-[var(--text-muted)]">
                  {isEditMode
              ...
              <div className="flex gap-2 flex-wrap shrink-0">
                <button type="button" onClick={() => router.back()} className="h-11 rounded-xl bg-white dark:bg-[#1E1F24] px-4 text-[13px] font-semibold text-[#86868b] dark:text-[var(--text-muted)] border border-[rgba(0,0,0,0.04)] transition-colors hover:bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E]">Back</button>
                {step === totalSteps && (
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <div className="inline-flex items-center gap-1.5 rounded-xl bg-white dark:bg-[#1E1F24] px-3 py-1.5 text-[12px] font-medium text-[#86868b] dark:text-[var(--text-muted)] shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
            ...
            <div className="mt-6 flex items-center gap-0">
              {STEPS.map((s, i) => (
                <div key={s.id} className="flex items-center flex-1">
                  <div className="flex flex-col items-center gap-1.5">
                    <div
                      ...
                      style={{
                        ...
                        border: step > s.id ? 'none' : `2px solid ${step === s.id ? '#1d1d1f' : 'var(--border)'}`,
                      }}
                  ...
                  {i < totalSteps - 1 && (
                    <div className="flex-1 h-[2px] mx-3 rounded-full" style={{ background: step > s.id ? '#22c55e' : 'var(--border)' }} />
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="flex gap-6 items-start">
              ...
              <form id="member-form" onSubmit={handleSubmit} onKeyDown={(e) => { if (e.key === 'Enter') e.preventDefault(); }}>
                <div className="space-y-5">
                  ...
                  <div className={`member-section-step ${step === 1 ? 'active' : ''}`}>
                    <div className="rounded-3xl bg-white dark:bg-[#1E1F24] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_12px_rgba(0,0,0,0.03)]">
                      <div className="mb-5 flex items-center justify-between gap-3 flex-wrap">
                        <div className="flex items-center gap-3">
                          <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] text-[#1d1d1f] dark:text-[var(--text-primary)]">
                            <User size={18} />
                          </span>
                          <div>
                            <div className="text-[16px] font-bold tracking-[-0.01em] text-[#1d1d1f] dark:text-[var(--text-primary)]">Personal Information</div>
                            <div className="text-[12px] text-[#86868b] dark:text-[var(--text-muted)]">Primary profile and contact details</div>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="sm:col-span-2">
                          ...
                          <div className="flex gap-2">
                            <div className="flex h-11 shrink-0 items-center gap-1 rounded-xl border border-[rgba(0,0,0,0.04)] bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] px-3 text-[13px] font-semibold text-[#86868b] dark:text-[var(--text-muted)]">+91</div>
                          ...
                          <label className="mt-1.5 flex items-center gap-2 cursor-pointer">
                            ...
                            <span className="text-[12px] text-[#86868b] dark:text-[var(--text-muted)]">Customer reluctant to share mobile number</span>
                          </label>
                        </div>
                        <div className="sm:col-span-2">
                          ...
                          <div className="flex gap-2">
                            {(['Male', 'Female', 'Other'] as const).map(g => (
                              <button key={g} type="button"
                                ...
                                style={{
                                  border: `1.5px solid ${f.gender === g ? '#dc2626' : 'var(--border)'}`,
                    ...
                    <div className="rounded-3xl bg-white dark:bg-[#1E1F24] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_12px_rgba(0,0,0,0.03)]">
                      <div className="mb-5 flex items-center justify-between gap-3 flex-wrap">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-[16px] font-bold tracking-[-0.01em] text-[#1d1d1f] dark:text-[var(--text-primary)]">Address & Contact</div>
                            <div className="text-[12px] text-[#86868b] dark:text-[var(--text-muted)]">Location details and emergency information</div>
                          </div>
                        </div>
                        <span className="rounded-full bg-[rgba(0,0,0,0.04)] px-3 py-1 text-[11px] font-bold text-[#86868b] dark:text-[var(--text-muted)]">Optional</span>
                      </div>
                    ...
                    <div className="rounded-3xl bg-white dark:bg-[#1E1F24] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_12px_rgba(0,0,0,0.03)]">
                      <div className="mb-5 flex items-center justify-between gap-3 flex-wrap">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-[16px] font-bold tracking-[-0.01em] text-[#1d1d1f] dark:text-[var(--text-primary)]">Fitness Profile</div>
                            <div className="text-[12px] text-[#86868b] dark:text-[var(--text-muted)]">Training preferences and goals</div>
                          </div>
                        </div>
                        <span className="rounded-full bg-[rgba(0,0,0,0.04)] px-3 py-1 text-[11px] font-bold text-[#86868b] dark:text-[var(--text-muted)]">Optional</span>
                      </div>
                ...
                {/* Bottom Bar */}
                <div className="mt-5 flex items-center justify-between gap-4 rounded-2xl bg-white dark:bg-[#1E1F24] px-5 py-4 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_12px_rgba(0,0,0,0.03)] flex-wrap">
                  <div>
                    <div className="text-[14px] font-semibold text-[#1d1d1f] dark:text-[var(--text-primary)]">{isEditMode ? 'Ready to update?' : 'Ready to create?'}</div>
                    <div className="text-[12px] text-[#86868b] dark:text-[var(--text-muted)]">Step {step} of {totalSteps} &middot; {STEPS[step - 1].label}</div>
                  </div>
                  <div className="flex gap-2">
                    {step > 1 && (
                      <button type="button" className="h-11 rounded-xl bg-white dark:bg-[#1E1F24] px-4 text-[13px] font-semibold text-[#86868b] dark:text-[var(--text-muted)] border border-[rgba(0,0,0,0.04)] transition-colors hover:bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] inline-flex items-center gap-1.5" onClick={() => setStep(s => s - 1)}>
                    ...
                    <button type="button" className="h-11 rounded-xl bg-white dark:bg-[#1E1F24] px-4 text-[13px] font-semibold text-[#86868b] dark:text-[var(--text-muted)] border border-[rgba(0,0,0,0.04)] transition-colors hover:bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E]" onClick={() => router.back()}>Cancel</button>
                  </div>
                </div>
              </form>
            </div>
            ...
            <div className="hidden xl:block w-[260px] shrink-0 sticky top-6 space-y-3">
              <div className="rounded-2xl bg-white dark:bg-[#1E1F24] p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_12px_rgba(0,0,0,0.03)]">
                <div className="flex items-center gap-2.5 mb-4">
                  <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] text-[#1d1d1f] dark:text-[var(--text-primary)]"><Sparkles size={15} /></span>
                  <div>
                    <div className="text-[13px] font-bold text-[#1d1d1f] dark:text-[var(--text-primary)]">AI Insights</div>
                    <div className="text-[10px] text-[#86868b] dark:text-[var(--text-muted)]">Onboarding intelligence</div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  >
                    <span className="flex h-[46px] w-[46px] items-center justify-center rounded-full bg-white dark:bg-[#1E1F24]">{aiScore}%</span>
                  </div>
                  <div>
                    <div className="text-[13px] font-bold text-[#1d1d1f] dark:text-[var(--text-primary)]">Profile Score</div>
                    <div className="text-[11px] text-[#86868b] dark:text-[var(--text-muted)]">{aiScore >= 70 ? 'Well on track!' : 'Add more details'}</div>
                  </div>
                </div>

                <div className="space-y-2">
                  <div className="flex items-center gap-2.5 rounded-xl bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] px-3 py-2.5">
                    ...
                    <div className="text-[11.5px] leading-snug text-[#86868b] dark:text-[var(--text-muted)]">
                      <span className="text-[9px] font-semibold uppercase tracking-[0.05em] text-[#86868b] dark:text-[var(--text-muted)]/60">Recommendation</span><br />
                      <strong className="text-[#1d1d1f] dark:text-[var(--text-primary)]">{f.gender ? (activeTrainers[0]?.name || 'Premium') : 'Personal'} Training</strong> — best match based on profile
                    </div>
                  </div>
                  <div className="flex items-center gap-2.5 rounded-xl bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] px-3 py-2.5">
                    ...
                    <div className="text-[11.5px] leading-snug text-[#86868b] dark:text-[var(--text-muted)]">
                      <span className="text-[9px] font-semibold uppercase tracking-[0.05em] text-[#86868b] dark:text-[var(--text-muted)]/60">Goal Suggestion</span><br />
                      <strong className="text-[#1d1d1f] dark:text-[var(--text-primary)]">{f.gender ? 'Strength & Conditioning' : 'General Fitness'}</strong> — popular with similar members
                    </div>
                  </div>
                  <div className="flex items-center gap-2.5 rounded-xl bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] px-3 py-2.5">
                    ...
                    <div className="text-[11.5px] leading-snug text-[#86868b] dark:text-[var(--text-muted)]">
                      <span className="text-[9px] font-semibold uppercase tracking-[0.05em] text-[#86868b] dark:text-[var(--text-muted)]/60">Retention Forecast</span><br />
                      <strong className="text-[#1d1d1f] dark:text-[var(--text-primary)]">{f.first_name ? '92% likelihood' : '—'}</strong> {f.first_name ? '6-month retention' : 'Complete profile for prediction'}
                    </div>
                  </div>
                  {f.interested_in && (
                    <div className="flex items-center gap-2.5 rounded-xl bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] px-3 py-2.5">
                      ...
                      <div className="text-[11.5px] leading-snug text-[#86868b] dark:text-[var(--text-muted)]">
                        <span className="text-[9px] font-semibold uppercase tracking-[0.05em] text-[#86868b] dark:text-[var(--text-muted)]/60">Interest Detected</span><br />
                        <strong className="text-[#1d1d1f] dark:text-[var(--text-primary)]">{f.interested_in}</strong> — relevant programs available
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="rounded-2xl bg-white dark:bg-[#1E1F24] p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_12px_rgba(0,0,0,0.03)]">
                <div className="flex items-center gap-2.5 mb-4">
                  <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#f5f5f7] dark:bg-[#1A1B1E] dark:bg-[#1A1B1E] text-[#1d1d1f] dark:text-[var(--text-primary)]"><Shield size={15} /></span>
                  <div>
                    <div className="text-[13px] font-bold text-[#1d1d1f] dark:text-[var(--text-primary)]">Onboarding Summary</div>
                    <div className="text-[10px] text-[#86868b] dark:text-[var(--text-muted)]">Quick overview</div>
                  </div>
                </div>
                <div className="space-y-2.5 text-[12px]">
                  {[
                    ...
                    <div key={k} className="flex justify-between">
                      <span className="text-[#86868b] dark:text-[var(--text-muted)]">{k}</span>
                      <span className="font-semibold text-[#1d1d1f] dark:text-[var(--text-primary)]">{v}</span>
                    </div>
                  ))}
                </div>
              </div>
