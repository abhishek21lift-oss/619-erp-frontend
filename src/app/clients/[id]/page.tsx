  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid rgba(255,255,255,${hovered ? 0.8 : 0.5})`,
        borderRadius: 24,
        boxShadow: hovered
...
/* ─── Action button ──────────────────────────────────────── */
function ActionBtn({ icon, label, onClick, gradient, outline = false, danger = false, disabled = false }: {
  icon: React.ReactNode; label: string; onClick?: () => void;
  gradient?: string; outline?: boolean; danger?: boolean; disabled?: boolean;
}) {
  ...
  if (error || !client) return (
    <Guard><AppShell>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '50vh', gap: 16 }}>
        ...
        <p style={{ color: '#64748b', fontSize: 15, fontWeight: 500 }}>{error || 'Member not found'}</p>
          ...
          <div style={{ position: 'relative', zIndex: 1, marginBottom: 20 }}>
            <button
              ...
              style={{
                ...
                borderRadius: 10, background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.6)',
          ...
          <div style={{ position: 'relative', zIndex: 1, marginBottom: 16 }}>
            <GlassCard style={{ padding: '28px 28px 24px' }} accent="#6366f1">
              <div className="hero-inner" style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
                    ...
                    {joinDate && (
                      <span style={{ padding: '4px 12px', borderRadius: 99, fontSize: 11, fontWeight: 600, background: 'rgba(15,23,42,0.05)', color: '#64748b', display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid rgba(226,232,240,0.7)' }}>
                ...
                >
                  <Link href={`/clients/${id}/add-subscription`} style={{ textDecoration: 'none' }}>
                    <ActionBtn icon={<Plus size={13} />} label="Add Membership" gradient="linear-gradient(135deg,#6366f1,#8b5cf6)" />
                  </Link>
                      ...
                      <thead>
                        <tr style={{ background: 'rgba(248,250,252,0.7)' }}>
                          {['Date', 'Check In', 'Check Out', 'Method', 'Duration'].map(h => (
                            <th key={h} style={{ padding: '10px 20px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {attendance.slice(0, 100).map((a, i) => {
                          ...
                          return (
                            <tr key={a.id} style={{ borderTop: '1px solid rgba(226,232,240,0.5)', background: i % 2 === 0 ? 'transparent' : 'rgba(248,250,252,0.3)' }}>
                              <td style={{ padding: '12px 20px', fontSize: 13, fontWeight: 600, color: '#0f172a' }}>{fmtDate(a.date)}</td>
                ...
                {payments.length === 0 ? (
                  ...
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      ...
                      <tbody>
                        {payments.map((p, i) => (
                          <tr key={p.id} style={{ borderTop: '1px solid rgba(226,232,240,0.5)', background: i % 2 === 0 ? 'transparent' : 'rgba(248,250,252,0.3)' }}>
                            <td style={{ padding: '12px 20px', fontSize: 13, fontWeight: 600, color: '#0f172a' }}>{fmtDate(p.date)}</td>
                            <td style={{ padding: '12px 20px', fontSize: 13, color: '#475569' }}>{p.plan}</td>
