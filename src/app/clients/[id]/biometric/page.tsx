...
function Inner() {
  ...
  useEffect(() => {
    api.clients.get(id)
      ...
      .catch((err) => {
        console.error('[biometric] client fetch failed', err);
      })
  ...
  if (loading) return (
    <AppShell>
      <div style={{ padding: '2rem', color: 'var(--text-disabled)', textAlign: 'center' }}>Loading member…</div>
    </AppShell>
  );
  ...
  return (
    <AppShell>
      ...
      <div className="bio-page bio-main" style={{ maxWidth: 760, margin: '0 auto', padding: '24px 20px 80px' }}>
        ...
        {/* Back */}
        <Link href={`/clients/${id}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)', fontSize: 13, fontWeight: 500, textDecoration: 'none', marginBottom: 24, padding: '6px 14px', borderRadius: 10, background: 'var(--bg-card)', border: '1px solid rgba(226,232,240,0.6)', backdropFilter: 'blur(12px)' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
            ...
            <div>
              <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-primary)', margin: 0, letterSpacing: '-0.02em' }}>Biometric Enrollment</h1>
              <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: 0 }}>Set up face &amp; fingerprint access for fast check-in</p>
            </div>
          </div>
        </div>

        {/* Member strip */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', borderRadius: 16, background: 'var(--bg-card)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.6)', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', marginBottom: 20 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-disabled)' }}>{client?.mobile ?? client?.phone ?? '—'}</div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
          {[
            ...
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 99, background: 'var(--bg-card)', border: '1px solid rgba(226,232,240,0.7)', fontSize: 11, fontWeight: 600, color, backdropFilter: 'blur(8px)' }}>
              {icon} {label}
            </div>
          ))}
        </div>
        ...
        ══════════════════════════════════════════════ */}
        <div style={{ marginBottom: 28 }}>
          <h2 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 12 }}>Face Recognition</h2>
        ...
        ══════════════════════════════════════════════ */}
        <div>
          <h2 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 12 }}>Fingerprint Device (optional)</h2>

          {/* Device connect card */}
          <div style={{ padding: '18px 20px', borderRadius: 18, background: 'var(--bg-card)', backdropFilter: 'blur(20px)', border: `1px solid ${deviceConnected ? 'rgba(16,185,129,0.3)' : 'rgba(226,232,240,0.6)'}`, boxShadow: '0 4px 20px rgba(0,0,0,0.06)', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>Biometric Device</div>
                <div style={{ fontSize: 12, color: deviceConnected ? '#059669' : 'var(--text-disabled)', display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {deviceConnected && (
            <div style={{ borderRadius: 20, background: 'var(--bg-card)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.6)', boxShadow: '0 4px 24px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
              <div style={{ padding: '16px 22px', borderBottom: '1px solid rgba(226,232,240,0.6)' }}>
                <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>Select finger to enroll</h3>
                <p style={{ fontSize: 12, color: 'var(--text-disabled)', margin: '4px 0 0' }}>Green = already enrolled</p>
              </div>
              <div style={{ padding: 20 }}>
                  ...
                  <div className="bio-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
                    {fingerLabels.map((label, fi) => (
                      <button key={fi} onClick={() => startScan(fi)} className="finger-btn"
                        style={{ padding: '10px 6px', borderRadius: 12, cursor: 'pointer', background: enrolledFingers.includes(fi) ? 'rgba(16,185,129,0.1)' : 'var(--bg-subtle)', border: `1px solid ${enrolledFingers.includes(fi) ? 'rgba(16,185,129,0.3)' : 'rgba(226,232,240,0.6)'}`, textAlign: 'center', transition: 'all 0.22s cubic-bezier(0.16,1,0.3,1)' }}
                      >
                        <Fingerprint size={16} color={enrolledFingers.includes(fi) ? '#059669' : 'var(--text-disabled)'} style={{ margin: '0 auto 4px' }} />
                        <div style={{ fontSize: 9, fontWeight: 600, color: enrolledFingers.includes(fi) ? '#059669' : 'var(--text-muted)', lineHeight: 1.3 }}>{label}</div>
                ...
                {fpStep === 'scanning' && (
                  <div style={{ textAlign: 'center', padding: '24px 0' }}>
                    ...
                    <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>Scanning {selectedFinger !== null ? fingerLabels[selectedFinger] : ''}…</p>
                ...
                {fpStep === 'success' && (
                  <div style={{ textAlign: 'center', padding: '20px 0' }}>
                    ...
                    <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>{selectedFinger !== null ? fingerLabels[selectedFinger] : 'Finger'} enrolled!</p>
                    <p style={{ fontSize: 12, color: 'var(--text-disabled)', marginBottom: 16 }}>{enrolledFingers.length} of 10 enrolled</p>
                ...
                {fpStep === 'error' && (
                  <div style={{ textAlign: 'center', padding: '20px 0' }}>
                    ...
                    <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>{fpError}</p>
