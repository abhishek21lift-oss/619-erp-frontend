...
export default function AssignPTPage() { return <Guard><Inner /></Guard>; }
...
function Inner() {
  ...
  const [form, setForm] = useState({ membership_plan: '', trainer_id: '', secondary_trainer_ids: [] as string[], total_sessions: '', start_date: '', end_date: '', base_price: '', selling_price: '', coupon: '', group_id: '' });
  ...
  function handlePlanSelect(planName: string) {
    ...
    setForm(f => {
      ...
      return { ...f, membership_plan: planName, start_date: start, end_date: computeEndDate(start, planName), base_price: f.base_price || (plan ? String(plan.base) : ''), selling_price: f.selling_price || (plan ? String(plan.final) : '') };
    });
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await api.clients.assignPt(id, {
        trainer_id: form.trainer_id, pt_start_date: form.start_date, pt_end_date: form.end_date,
        membership_plan: form.membership_plan || null, sessions: form.total_sessions ? parseInt(form.total_sessions) : null,
        amount: parseFloat(form.selling_price) || 0, payment_method: paymentMethod,
      });
      ...
      const m = err?.message || 'Failed to assign PT'; setError(m); toast.error(m);
  ...
  return (
    <AppShell>
      <WorkflowLayout
        hero={
          <WorkflowHero client={client} backHref={`/clients/${id}`} badge={{ label: 'Assign Personal Training', color: '#f59e0b' }} />
        }
        rail={
          <SummaryRail title="PT Summary" items={summaryItems} total={total} client={client} />
        }
        actionBar={
          <StickyActionBar total={total} label="Assign Personal Training" saving={saving} onCancel={() => router.push(`/clients/${id}`)} />
        }
      ...
      >
        ...
        <GlassCard>
          <SectionHeading eyebrow="PT PLAN" title="Select PT Membership" />
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {ptPlans.map(p => (
              <button key={p.name} type="button" onClick={() => handlePlanSelect(p.name)}
                style={{ display: 'flex', flexDirection: 'column', gap: 2, padding: '12px 18px', borderRadius: 12, border: form.membership_plan === p.name ? '2px solid #f59e0b' : '2px solid var(--border, #e5e7eb)', background: form.membership_plan === p.name ? '#fffbeb' : '#fff', cursor: 'pointer', transition: 'all .18s', minWidth: 130, textAlign: 'left' }}>
                <span style={{ fontWeight: 700, fontSize: 14, color: form.membership_plan === p.name ? '#92400e' : 'var(--text, #1f2937)' }}>{p.name}</span>
                <span style={{ fontSize: 13, color: form.membership_plan === p.name ? '#d97706' : 'var(--muted)' }}>₹{p.final.toLocaleString('en-IN')}</span>
              </button>
            ))}
          </div>
        </GlassCard>
        ...
        <GlassCard>
          ...
          <div style={{ marginTop: 8 }}>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--muted)', letterSpacing: '.04em' }}>Secondary Trainer(s) <span style={{ fontWeight: 400, textTransform: 'none' }}>(hold Ctrl/⌘ to multi-select)</span></span>
