          ...
          <motion.div initial={{ opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
              {[{ label: 'From', value: client?.freeze_from?.slice(0, 10) || '—' }, { label: 'Until', value: client?.freeze_until?.slice(0, 10) || '—' }].map(r => (
                <div key={r.label} style={{ background: 'var(--bg-white)', borderRadius: 8, padding: '10px 14px' }}>
            ...
            <button type="button" onClick={handleUnfreeze} disabled={saving}
              style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '10px 20px', background: '#2563eb', color: 'var(--bg-white)', border: 'none', borderRadius: 9, fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>
