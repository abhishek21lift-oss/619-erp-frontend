...
function ClientsContent() {
  ...
  const statusCounts = {
    ...
    dues:    clients.filter(c=>c.balance_amount>0).length,
  };
                    ...
                    {clients.map(c => {
                      ...
                      return (
                        <tr key={c.id} style={{opacity: deleting===c.id ? .4 : 1}}>
                          <td><span className="mono text-muted">{c.client_id}</span></td>
                          <td>
                          ...
                          <td style={{color:c.balance_amount>0?'var(--danger)':'var(--muted)',fontWeight:c.balance_amount>0?700:400}}>
