...
function KpiCard({ label, value, icon, gradient, colorIndex }: {
  ...
  return (
    <m.div variants={itemVariants}
      ...
      <m.div style={{ position: 'absolute', bottom: -20, left: -20, width: 80, height: 80, borderRadius: '50%', background: `radial-gradient(circle, rgba(255,255,255,0.06), transparent)`, pointerEvents: 'none' }}
  ...
  return (
    <AppShell>
      ...
      <div style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg, #0f0a1e 0%, #1a1040 25%, #0f172a 55%, #1e0a2e 80%, #0f0a1e 100%)', padding: '52px 32px 44px', borderRadius: '0 0 40px 40px' }}>
        <m.div style={{ position: 'absolute', top: -100, right: -60, width: 350, height: 350, borderRadius: '50%', background: 'radial-gradient(circle, rgba(139,92,246,0.2), transparent 70%)', pointerEvents: 'none' }}
          animate={{ x: [0, 30, -20, 0], y: [0, -40, 20, 0] }} transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut' }} />
        <m.div style={{ position: 'absolute', bottom: -80, left: -40, width: 280, height: 280, borderRadius: '50%', background: 'radial-gradient(circle, rgba(236,72,153,0.12), transparent 70%)', pointerEvents: 'none' }}
          animate={{ x: [0, -25, 35, 0], y: [0, 30, -15, 0] }} transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }} />
        <m.div style={{ position: 'absolute', top: '20%', left: '60%', width: 200, height: 200, borderRadius: '50%', background: 'radial-gradient(circle, rgba(16,185,129,0.08), transparent 70%)', pointerEvents: 'none' }}
