        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              ...
              <h1 style={{ fontSize: 24, fontWeight: 800, color: '#0f172a', margin: 0, letterSpacing: '-0.02em' }}>Pending Dues</h1>
            </div>
            <p style={{ fontSize: 14, color: '#64748b', margin: 0 }}>Collections dashboard · Outstanding balances · Recovery pipeline</p>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 12, position: 'relative', flexWrap: 'wrap' }}>
            <div style={{ background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(10px)', borderRadius: 14, padding: '14px 18px', border: '1px solid rgba(255,255,255,0.15)', textAlign: 'center', minWidth: 90 }}>
              ...
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', marginTop: 2 }}>High Risk</div>
            </div>
            <div style={{ background: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(10px)', borderRadius: 14, padding: '14px 18px', border: '1px solid rgba(255,255,255,0.15)', textAlign: 'center', minWidth: 90 }}>
              <div style={{ fontSize: 22, fontWeight: 900, color: 'white' }}>{medRisk}</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', marginTop: 2 }}>Medium</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 20, border: '1px solid #f1f5f9', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
          {loading ? (
            ...
            <div style={{ padding: '72px 32px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
              ...
              <div>
                <p style={{ fontSize: 20, fontWeight: 800, color: '#0f172a', margin: '0 0 6px' }}>All Clear! 🎉</p>
                <p style={{ fontSize: 14, color: '#64748b', margin: 0 }}>{search ? 'No members match your search.' : 'Every member is up to date — no pending dues.'}</p>
              </div>
            </div>
          ) : (
                      ...
                      <tr key={d.id} style={{ borderBottom: '1px solid #f8fafc', transition: 'background 150ms' }}>
                        <td style={{ padding: '14px 16px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div>
                              <div style={{ fontWeight: 600, color: '#0f172a', fontSize: 14 }}>{d.name}</div>
                              <div style={{ fontSize: 11, fontFamily: 'monospace', color: '#94a3b8' }}>{d.client_id || '—'}</div>
                            </div>
                          </div>
                        </td>
                        <td style={{ padding: '14px 16px', fontSize: 13, color: '#64748b', fontFamily: 'monospace' }}>{d.mobile || '—'}</td>
                        <td style={{ padding: '14px 16px', fontSize: 13, color: '#64748b' }}>{d.trainer_name || '—'}</td>
                        <td style={{ padding: '14px 16px' }}>
                          <span style={{ fontSize: 16, fontWeight: 800, color: '#ef4444', letterSpacing: '-0.02em' }}>{fmt(amt)}</span>
                        </td>
                        <td style={{ padding: '14px 16px', fontSize: 13, color: '#64748b' }}>{fmtDate(d.pt_end_date)}</td>
                        <td style={{ padding: '14px 16px' }}>
