      ...
      <div className="collection-page" style={{ maxWidth: 1280, margin: '0 auto', padding: '24px 20px 48px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              ...
              <h1 style={{ fontSize: 24, fontWeight: 800, color: '#0f172a', margin: 0, letterSpacing: '-0.02em' }}>Collection</h1>
            </div>
            <p style={{ fontSize: 14, color: '#64748b', margin: 0 }}>Monthly revenue collected · Payment volume · {year} view</p>
          </div>
          {/* Year selector */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button onClick={() => setYear(y => y - 1)}
              style={{ width: 34, height: 34, borderRadius: 10, border: '1.5px solid #e2e8f0', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b' }}>
              ‹
            </button>
            <span style={{ fontSize: 15, fontWeight: 700, minWidth: 48, textAlign: 'center', color: '#0f172a' }}>{year}</span>
            <button onClick={() => setYear(y => y + 1)}
              disabled={year >= new Date().getFullYear()}
              style={{ width: 34, height: 34, borderRadius: 10, border: '1.5px solid #e2e8f0', background: 'white', cursor: year >= new Date().getFullYear() ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: year >= new Date().getFullYear() ? '#cbd5e1' : '#64748b' }}>
              ›
            </button>
          </div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 20, border: '1px solid #f1f5f9', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: '#0f172a' }}>Monthly Breakdown — {year}</div>
            <div style={{ fontSize: 12, color: '#94a3b8' }}>Avg: <strong style={{ color: '#10b981' }}>{fmtCompact(avg)}</strong> / month</div>
          </div>
          {loading ? (
              ...
              <table className="collection-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                    {['Month', 'Payments', 'Collected', 'Bar'].map((h) => (
                      <th key={h} style={{ padding: '14px 20px', textAlign: h === 'Collected' ? 'right' : 'left', fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', background: '#fafbff', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {fullYear.map((m, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #f8fafc', transition: 'background 150ms', opacity: m.revenue === 0 ? 0.45 : 1 }}>
                      ...
                      <td style={{ padding: '14px 20px', color: '#64748b', fontVariantNumeric: 'tabular-nums' }}>{m.count || '—'}</td>
                ...
                <tfoot>
                  <tr style={{ borderTop: '2px solid #f1f5f9', background: '#fafbff' }}>
                    <td style={{ padding: '14px 20px', fontWeight: 800, color: '#0f172a' }}>Total</td>
                    <td style={{ padding: '14px 20px', fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: '#0f172a' }}>{totalCount}</td>
