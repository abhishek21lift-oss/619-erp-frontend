    ...
    <Guard>
      <AppShell title="AI Business Insights">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">AI-powered analysis of your studio&apos;s performance and growth opportunities</p>
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {PERIODS.map((p) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${period === p.value ? 'bg-primary text-primary-foreground' : 'border border-border hover:bg-muted'}`}
              >
                {p.label}
              </button>
            ))}
      ...
      {insights && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="p-5 rounded-2xl border border-border bg-card">
            ...
            <p className="text-sm text-muted-foreground mt-2">{insights.executive_summary || insights.summary}</p>
            {insights.period && (
              <div className="mt-2 text-xs text-muted-foreground">Period: <strong className="text-foreground">{insights.period}</strong></div>
            )}
          </div>
              ...
              {insights.trends.map((t, i) => (
                <div key={i} className="flex items-center gap-3 py-2 border-b border-border/50 last:border-0">
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-medium text-foreground">{t.metric}</div>
                    <div className="text-xs text-muted-foreground">{t.insight}</div>
                  </div>
                  <span className={`text-sm font-semibold ${t.direction === 'up' || t.direction === 'increasing' ? 'text-green-600' : t.direction === 'down' || t.direction === 'decreasing' ? 'text-red-600' : 'text-muted-foreground'}`}>
          ...
          {insights.opportunities?.length ? (
            <div className="p-5 rounded-2xl border border-green-200 dark:border-green-900 bg-green-50 dark:bg-green-950/30 space-y-3">
              <h3 className="font-semibold text-green-700 dark:text-green-400">Growth Opportunities</h3>
              {insights.opportunities.map((o, i) => (
                <div key={i} className="flex gap-3">
                  <div className="w-6 h-6 rounded-full bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-400 text-xs font-bold flex items-center justify-center shrink-0">{i + 1}</div>
                  <div>
                    <div className="text-sm font-medium text-green-800 dark:text-green-300">{o.opportunity}</div>
            ...
            <div className="p-5 rounded-2xl border border-yellow-200 dark:border-yellow-900 bg-yellow-50 dark:bg-yellow-950/30 space-y-3">
              ...
              {insights.risks.map((r, i) => (
                <div key={i} className="border-b border-yellow-200 dark:border-yellow-900 pb-3 last:border-0 last:pb-0">
                  ...
                  <p className="text-xs text-yellow-700 dark:text-yellow-400 mt-1">{r.recommended_action}</p>
                </div>
              ))}
            </div>
          ) : null}
            ...
            <div className="p-5 rounded-2xl border border-border bg-card space-y-3">
              ...
              {insights.recommendations
                ...
                <div key={i} className="flex items-start gap-3 py-2 border-b border-border/50 last:border-0">
                  <div className="w-6 h-6 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">{r.priority}</div>
                  <div className="flex-1">
                  ...
                  {r.timeframe && <div className="text-xs text-muted-foreground shrink-0">{r.timeframe}</div>}
                </div>
              ))}
            </div>
          ) : null}
