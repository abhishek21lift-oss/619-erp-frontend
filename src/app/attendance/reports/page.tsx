...
export default function AttendanceReportsPage() {
  ...
  const traffic = useAsync(() => api.reports.revenue({ period: dateRange }), [dateRange]);
  const monthly = useAsync(() => api.reports.monthly(new Date().getFullYear()), []);
        ...
        <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
            ...
            {['7', '30', '90'].map(d => (
              <button key={d} onClick={() => setDateRange(d)}
                ...
                style={{
                  ...
                  border: `1.5px solid ${dateRange === d ? '#0284c7' : 'rgba(0,0,0,0.06)'}`,
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 rounded-[20px] p-6"
              style={{ background: 'rgba(255,255,255,0.75)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.95)', boxShadow: '0 2px 20px rgba(15,23,42,0.06)' }}>
              ...
              {traffic.loading ? (
                ...
                <div className="h-64 flex items-center justify-center" style={{ color: 'rgb(148,163,184)' }}>
                  <p className="text-sm">Attendance chart loads here with Recharts</p>
                </div>
              )}
            </div>
            ...
            <div className="rounded-[20px] p-6"
              ...
              <div className="space-y-4">
                {[
                  { label: 'Avg Daily Check-Ins', value: '—', change: '+12%', up: true },
                  { label: 'Peak Hour', value: '—', change: '7-9 AM', up: true },
                  { label: 'Missed Check-Outs', value: '—', change: '3 today', up: false },
                  { label: 'Staff Exceptions', value: '—', change: '1 pending', up: false },
                ].map(m => (
                  <div key={m.label} className="flex items-center justify-between py-2 border-b" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
                    <div>
          ...
          <div className="mt-6 rounded-[20px] p-6"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-[13px]">
                ...
                <tbody>
                  {(monthly.data as any[] || []).slice(0, 6).map((m: any, i: number) => (
                    <tr key={i} style={{ borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                      <td className="py-3 font-medium" style={{ color: 'rgb(15,23,42)' }}>{m.month || m.label || '—'}</td>
                      <td className="py-3" style={{ color: 'rgb(100,116,139)' }}>{m.total || m.count || '—'}</td>
                      <td className="py-3" style={{ color: 'rgb(100,116,139)' }}>—</td>
                      <td className="py-3" style={{ color: 'rgb(100,116,139)' }}>—</td>
                      <td className="py-3" style={{ color: 'rgb(100,116,139)' }}>—</td>
                    </tr>
                  ))}
