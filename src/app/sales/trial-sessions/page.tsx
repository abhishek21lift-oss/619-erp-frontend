...
import { CalendarCheck, Plus, Loader2, CheckCircle2 } from 'lucide-react';
...
export default function TrialSessionsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSuccess('');
    try {
      await api.leads.update(leadId, { status: 'trial_booked', scheduled_at: scheduledAt, trainer_id: trainerId || undefined, notes: notes || undefined });
      setLeadId(''); setScheduledAt(''); setTrainerId(''); setNotes('');
      setSuccess('Trial scheduled!');
      leads.refetch();
            ...
            <div className="rounded-[20px] p-6" style={{ background: 'var(--bg-card)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.95)' }}>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <button type="submit" disabled={!leadId || !scheduledAt || saving}
                  className="w-full rounded-[14px] py-3 font-[700] text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                  style={{ background: !leadId || !scheduledAt || saving ? 'rgba(0,0,0,0.1)' : 'linear-gradient(135deg, #0369a1, #0ea5e9)', color: '#fff', border: 'none', cursor: saving ? 'not-allowed' : 'pointer' }}>
                  {saving ? <Loader2 size={16} className="animate-spin" /> : <Plus size={16} />} Schedule Trial
                </button>
                {success && (
                  <div className="flex items-center gap-2 mt-3 text-[13px] font-semibold text-emerald-600">
