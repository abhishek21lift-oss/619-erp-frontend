...
import { TrendingUp, Users, UserCheck, Activity, Zap, Target, ArrowRight, Sparkles, BarChart2, ChevronDown } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell } from 'recharts';

export default function ConversionFunnelPage() { return <Guard><Inner /></Guard>; }
...
function Inner() {
  ...
  useEffect(() => {
    api.clients.list({}).then(r => setClients(r)).catch(e => setError(e.message)).finally(() => setLoading(false));
  }, []);
      ...
      <div className="relative" style={{ zIndex: 1, padding: '1.5rem 1.75rem', maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-0.03em', color: '#0f172a' }}>Conversion Funnel</h1>
          </div>
          <p style={{ fontSize: 13, color: '#64748b', marginLeft: 40 }}>Track how leads move through your sales pipeline</p>
        </div>
        ...
        <div className="grid gap-4 mb-6" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
          {[
            ...
            >
              ...
              <div style={{ fontSize: 28, fontWeight: 800, letterSpacing: '-0.04em', color: '#0f172a', lineHeight: 1 }}>{k.value}</div>
        ...
        <div className="grid gap-6" style={{ gridTemplateColumns: '1fr 320px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.8)', borderRadius: 24, padding: '1.5rem', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', marginBottom: 20 }}>Pipeline Stages</div>
              {loading ? (
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div style={{ fontSize: 18, fontWeight: 800, color: '#0f172a', fontVariantNumeric: 'tabular-nums' }}>{s.value}</div>
                          </div>
                        </div>
            ...
            {/* Bar chart */}
            <div style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.8)', borderRadius: 24, padding: '1.5rem', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', marginBottom: 16 }}>Stage Comparison</div>
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={chartData} barSize={40}>
                  <XAxis dataKey="name" tick={{ fontSize: 11, fill: '#94a3b8', fontWeight: 600 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
                  <Tooltip contentStyle={{ background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 10, fontSize: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.8)', borderRadius: 20, padding: '1.25rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#0f172a' }}>AI Insights</div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {insights.map((ins, i) => (
                  <motion.div key={i}
                  ...
                  >
                    <div style={{ fontSize: 14, marginBottom: 4 }}>{ins.icon} <span style={{ fontWeight: 700, fontSize: 12, color: '#0f172a' }}>{ins.title}</span></div>
                    <p style={{ fontSize: 12, color: '#64748b', lineHeight: 1.55, margin: 0 }}>{ins.body}</p>
                  </motion.div>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.06), rgba(14,165,233,0.04))', border: '1px solid rgba(16,185,129,0.12)', borderRadius: 20, padding: '1.25rem' }}>
              ...
              <div style={{ fontSize: 44, fontWeight: 900, color: '#0f172a', letterSpacing: '-0.05em', lineHeight: 1 }}>{convRate}<span style={{ fontSize: 20 }}>%</span></div>
              <p style={{ fontSize: 12, color: '#64748b', marginTop: 6 }}>Industry benchmark: 25–40%</p>
