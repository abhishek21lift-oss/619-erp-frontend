...
import { Check, ChevronRight, ChevronLeft, User, Phone, Mail, Cake, Dumbbell, MapPin, FileText, Sparkles } from 'lucide-react';
...
function Inner() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      timerRef.current = setTimeout(() => router.push('/sales/leads'), 900);
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-0.03em', color: 'var(--text-primary)' }}>New Enquiry</h1>
            </div>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginLeft: 40 }}>Add a new lead to your CRM pipeline</p>
          </div>
          <button onClick={() => router.back()} style={{ padding: '8px 16px', borderRadius: 10, border: '1px solid var(--border-2)', background: 'var(--bg-card)', color: 'var(--text-muted)', fontSize: 13, cursor: 'pointer', fontWeight: 600 }}>← Back</button>
        </div>

        <div className="grid gap-6" style={{ gridTemplateColumns: '1fr 300px' }}>
          {/* LEFT: Form */}
          <form onSubmit={handleSubmit} style={{ background: 'var(--bg-card)', backdropFilter: 'blur(16px)', border: '1px solid var(--border)', borderRadius: 24, padding: '1.75rem', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
                ...
                <React.Fragment key={s.id}>
                  <button onClick={() => setStep(s.id)}
                    style={{
                      ...
                      background: step === s.id ? 'linear-gradient(135deg,#7c3aed,#4f46e5)' : step > s.id ? 'linear-gradient(135deg,#10b981,#0ea5e9)' : 'var(--bg-subtle)',
                      color: step >= s.id ? '#fff' : 'var(--text-muted)',
                    }}
                  >
                    {step > s.id ? <Check size={12} /> : s.icon} {s.label}
                  </button>
                  {i < STEPS.length - 1 && <div style={{ width: 20, height: 1, background: step > s.id ? '#7c3aed' : 'var(--border-2)' }} />}
                </React.Fragment>
              ))}
            </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {step === 1 && (
                  <div>
                    <h3 style={{ fontWeight: 700, fontSize: 16, color: 'var(--text-primary)', marginBottom: 4 }}>Personal Details</h3>
                    <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>Basic contact information</p>
                ...
                {step === 2 && (
                  <div>
                    <h3 style={{ fontWeight: 700, fontSize: 16, color: 'var(--text-primary)', marginBottom: 4 }}>What are they interested in?</h3>
                    <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>Select the primary fitness goal</p>
                    <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))' }}>
                      {INTERESTS.map(int => (
                        <button key={int.value} type="button" onClick={() => set('interested_in', int.value)}
                          style={{
                            padding: '14px 12px', borderRadius: 14, border: form.interested_in === int.value ? `2px solid ${int.color}` : '2px solid var(--border-2)',
                            background: form.interested_in === int.value ? `${int.color}12` : 'var(--bg-card)',
                        ...
                        >
                          ...
                          <span style={{ fontSize: 12, fontWeight: 700, color: form.interested_in === int.value ? int.color : 'var(--text-muted)' }}>{int.label}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {step === 3 && (
                  <div>
                    <h3 style={{ fontWeight: 700, fontSize: 16, color: 'var(--text-primary)', marginBottom: 4 }}>How did they find you?</h3>
                    <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>Track your best lead acquisition channels</p>
                    <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))' }}>
                      {SOURCES.map(src => (
                        <button key={src.value} type="button" onClick={() => set('source', src.value)}
                          style={{
                            padding: '16px 12px', borderRadius: 14, border: form.source === src.value ? `2px solid ${src.color}` : '2px solid var(--border-2)',
                            background: form.source === src.value ? `${src.color}12` : 'var(--bg-card)',
                            cursor: 'pointer', transition: 'all 0.18s', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6
                          }}
                        >
                          <span style={{ fontSize: 24 }}>{src.icon}</span>
                          <span style={{ fontSize: 12, fontWeight: 700, color: form.source === src.value ? src.color : 'var(--text-muted)' }}>{src.label}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {step === 4 && (
                  <div>
                    <h3 style={{ fontWeight: 700, fontSize: 16, color: 'var(--text-primary)', marginBottom: 4 }}>Notes & Summary</h3>
                    <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>Any additional context about this lead</p>
                    <textarea
                      value={form.notes}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24, paddingTop: 20, borderTop: '1px solid var(--border)' }}>
              <button onClick={() => step > 1 ? setStep(s => s - 1) : router.back()}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 18px', borderRadius: 11, border: '1px solid var(--border-2)', background: 'var(--bg-card)', color: 'var(--text-muted)', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.18s' }}
              >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {/* Lead Score */}
            <div style={{ background: 'var(--bg-card)', backdropFilter: 'blur(16px)', border: '1px solid var(--border)', borderRadius: 20, padding: '1.25rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
                <Sparkles size={14} style={{ color: '#7c3aed' }} />
                <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '0.04em', textTransform: 'uppercase' }}>Lead Quality Score</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-disabled)' }}>out of 100</div>
                </div>
              </div>
            ...
            {/* Live preview */}
            <div style={{ background: 'var(--bg-card)', backdropFilter: 'blur(16px)', border: '1px solid var(--border)', borderRadius: 20, padding: '1.25rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', flex: 1 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-disabled)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 12 }}>Live Preview</div>
...
const inputStyle: React.CSSProperties = {
  ...
  border: '1.5px solid var(--border-2)', background: 'var(--bg-subtle)',
...
function Field({ label, icon, children }: { label: string; icon?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div>
      <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>
        {icon} {label}
      </label>
      {children}
    </div>
...
function PreviewRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--border)' }}>
      <span style={{ fontSize: 11, color: 'var(--text-disabled)', fontWeight: 600 }}>{label}</span>
      <span style={{ fontSize: 12, color: 'var(--text-primary)', fontWeight: 700, maxWidth: '60%', textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{value}</span>
    </div>
  );
}
