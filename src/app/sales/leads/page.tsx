...
import {
  Search, Plus, Flame, Thermometer, Snowflake,
  MessageCircle, UserCheck, ChevronRight, Zap,
  TrendingUp, Users, ArrowUpRight,
  LayoutGrid, List, Sparkles, RefreshCw, Filter,
} from 'lucide-react';
import { SearchInput, FilterChips, StatCard, PageHeader } from '@/components/ui';
...
function Inner() {
  ...
  const leads = useMemo(() => {
    const pool = clients.filter(c => c.status === 'lead');
  ...
  const all = useMemo(() => clients.filter(c => c.status === 'lead'), [clients]);
  ...
  return (
    <AppShell>
      ...
      <div className="relative" style={{ zIndex: 1, padding: '1.5rem 1.75rem', maxWidth: 1280, margin: '0 auto' }}>
        ...
        <PageHeader
          title="Lead Inbox"
          subtitle={`${all.length} total leads · ${hot.length} hot · ${today.length} today`}
          actions={
            <div className="flex items-center gap-3">
              <button
                onClick={load}
                disabled={refreshing}
                className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium border border-[var(--border-2)] bg-[var(--bg-white)] text-[var(--text-muted)] cursor-pointer transition-all duration-200 hover:bg-[var(--bg-hover)]"
              >
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          <SearchInput
            ...
            containerClassName="min-w-[240px] max-w-[380px] flex-1"
          />
          {/* Temp filters */}
          <FilterChips
            chips={[
            ...
            value={filterTemp}
            onChange={(id) => setFilterTemp(id as Temp | 'all')}
          />
          {/* View toggle */}
          <div className="flex items-center gap-1 ml-auto" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-2)', borderRadius: 10, padding: 3 }}>
            {(['list', 'grid'] as const).map(v => (
              <button key={v} onClick={() => setView(v)}
                style={{
                  padding: '5px 10px', borderRadius: 7, border: 'none', cursor: 'pointer', transition: 'all 0.15s',
                  background: view === v ? '#7c3aed' : 'transparent', color: view === v ? '#fff' : 'var(--text-disabled)',
                }}
              >
          ...
          <div style={{ fontSize: 12, color: 'var(--text-disabled)', fontWeight: 600 }}>{leads.length} leads</div>
        </div>
        ...
        {loading ? (
          <div className="grid gap-3" style={{ gridTemplateColumns: view === 'grid' ? 'repeat(auto-fill,minmax(300px,1fr))' : '1fr' }}>
            {[...Array(6)].map((_, i) => (
              <div key={i} className="rounded-[16px] p-4 animate-pulse" style={{
                height: 72,
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            style={{ textAlign: 'center', padding: '4rem 2rem', background: 'var(--bg-card)', borderRadius: 24, border: '1px solid var(--border)' }}
          >
            <div style={{ fontSize: 48, marginBottom: 12 }}>🎯</div>
            <h3 style={{ fontWeight: 700, fontSize: 18, color: 'var(--text-primary)', marginBottom: 6 }}>Inbox Zero</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>All leads have been actioned. Add a new enquiry to get started.</p>
            <Link href="/sales/enquiry"
          ...
          /* List View */
          <div style={{ background: 'var(--bg-card)', backdropFilter: 'blur(16px)', border: '1px solid var(--border)', borderRadius: 20, overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
            {/* Table header */}
            <div style={{ display: 'grid', gridTemplateColumns: '2.5fr 1.2fr 1.4fr 1fr 1fr 100px', gap: '0 1rem', padding: '0.65rem 1.25rem',
              ...
              {['Lead', 'Mobile', 'Interest', 'Source', 'Added', ''].map(h => (
                <div key={h} style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-disabled)' }}>{h}</div>
              ))}
            </div>
            <AnimatePresence>
              {leads.map((c, i) => {
                ...
                return (
                  <motion.div
                    ...
                    style={{ display: 'grid', gridTemplateColumns: '2.5fr 1.2fr 1.4fr 1fr 1fr 100px', gap: '0 1rem',
                      padding: '0.85rem 1.25rem', borderBottom: '1px solid var(--border)', alignItems: 'center', cursor: 'default' }}
                  >
                    ...
                    {/* Mobile */}
                    <div style={{ fontSize: 13, color: 'var(--text-muted)', fontVariantNumeric: 'tabular-nums' }}>{c.mobile || '—'}</div>
                    {/* Interest */}
                    <div style={{ fontSize: 12 }}>
                      {c.interested_in ? (
                          <span style={{ padding: '3px 10px', borderRadius: 20, background: 'rgba(99,102,241,0.08)', color: '#6366f1', fontWeight: 600, fontSize: 11 }}>{c.interested_in}</span>
                    ...
                    {/* Date */}
                    <div style={{ fontSize: 12, color: 'var(--text-disabled)', fontVariantNumeric: 'tabular-nums' }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      {c.mobile && (
                        <a href={`https://wa.me/91${c.mobile}`} target="_blank" rel="noopener noreferrer"
                          style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(22,163,74,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                          title="WhatsApp"
                      ...
                      <Link href={`/clients/${c.id}`}
                        style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(99,102,241,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                        title="Open"
                      >
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 12, color: 'var(--text-disabled)' }}>{c.mobile || '—'}</div>
                        </div>
                      </div>
                      <div style={{
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      {c.mobile && (
                        <a href={`https://wa.me/91${c.mobile}`} target="_blank" rel="noopener noreferrer"
                          style={{ flex: 1, padding: '7px 0', borderRadius: 10, background: 'rgba(22,163,74,0.08)', color: '#16a34a', fontSize: 12, fontWeight: 700, textAlign: 'center', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}
                        >
                          <MessageCircle size={12} /> WhatsApp
                        </a>
                      )}
                      <Link href={`/clients/${c.id}`}
                        style={{ flex: 1, padding: '7px 0', borderRadius: 10, background: 'rgba(99,102,241,0.08)', color: '#6366f1', fontSize: 12, fontWeight: 700, textAlign: 'center', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}
                      >
