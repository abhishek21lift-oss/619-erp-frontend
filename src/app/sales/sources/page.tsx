...
import { TrendingUp, TrendingDown, Sparkles, PieChart as PieIcon } from 'lucide-react';
...
const SOURCE_META: Record<string, { icon: string; color: string; gradient: string }> = {
  ...
  'Other':            { icon: '📌', color: 'var(--text-muted)', gradient: 'from-slate-400 to-slate-500' },
};

function getMeta(src: string) {
  return SOURCE_META[src] || { icon: '📌', color: 'var(--text-muted)', gradient: 'from-slate-400 to-slate-500' };
}

export default function LeadSourcesPage() { return <Guard role="admin"><Inner /></Guard>; }
      ...
      <div className="relative" style={{ zIndex: 1, padding: '1.5rem 1.75rem', maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-0.03em', color: 'var(--text-primary)' }}>Lead Sources</h1>
          </div>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginLeft: 40 }}>Marketing intelligence — understand where your best leads come from</p>
        </div>
        ...
        <div className="grid gap-4 mb-6" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
          {kpis.map((k, i) => (
            ...
            >
              ...
              <div style={{ fontSize: k.small ? 16 : 26, fontWeight: 800, letterSpacing: k.small ? '-0.01em' : '-0.04em', color: 'var(--text-primary)', lineHeight: 1.2 }}>{k.value}</div>
              ...
              <div style={{ fontSize: 11, color: 'var(--text-disabled)', marginTop: 2 }}>{k.sub}</div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid gap-6" style={{ gridTemplateColumns: '1fr 1fr' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ background: 'var(--bg-card)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.8)', borderRadius: 24, padding: '1.5rem', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 16 }}>Source Breakdown</div>
              {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-disabled)' }}>
                  ...
                  <div style={{ fontWeight: 700, color: 'var(--text-primary)' }}>No data yet</div>
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div>
                              <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-primary)' }}>{r.source}</div>
                              <div style={{ fontSize: 11, color: 'var(--text-disabled)' }}>{r.total} leads · {r.converted} converted</div>
                            </div>
                          </div>
                          <div style={{ textAlign: 'right' }}>
                            <div style={{
                              ...
                              {r.rate >= 40 ? <TrendingUp size={12} /> : r.rate >= 20 ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
                              {r.rate}%
                            </div>
                            <div style={{ fontSize: 10, color: 'var(--text-disabled)' }}>{sharePct}% share</div>
                          </div>
                        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {/* Pie chart */}
            <div style={{ background: 'var(--bg-card)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.8)', borderRadius: 24, padding: '1.5rem', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8 }}>Lead Distribution</div>
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={3} dataKey="value">
                    {pieData.map((entry) => <Cell key={entry.name} fill={entry.color} />)}
                  </Pie>
                  <Tooltip contentStyle={{ background: 'var(--bg-card)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 10, fontSize: 12 }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
            ...
            {/* Conv rate bar */}
            <div style={{ background: 'var(--bg-card)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.8)', borderRadius: 24, padding: '1.5rem', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 12 }}>Conversion % by Source</div>
              <ResponsiveContainer width="100%" height={140}>
                <BarChart data={barData} barSize={28}>
                  <XAxis dataKey="name" tick={{ fontSize: 11, fill: 'var(--text-disabled)', fontWeight: 600 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 10, fill: 'var(--text-disabled)' }} axisLine={false} tickLine={false} unit="%" />
                  <Tooltip contentStyle={{ background: 'var(--bg-card)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 10, fontSize: 12 }} />
                  <Bar dataKey="conv" radius={[6, 6, 0, 0]}>
                    {barData.map((entry) => <Cell key={entry.name} fill={getMeta(breakdown.find(b => b.source.split(' ')[0] === entry.name)?.source || '').color} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div style={{ background: 'var(--bg-card)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.8)', borderRadius: 20, padding: '1.25rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-primary)' }}>Marketing Insights</div>
              </div>
              {insights.map((ins, i) => (
                ...
                >
                  <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 3 }}>{ins.icon} {ins.title}</div>
                  <p style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.55, margin: 0 }}>{ins.body}</p>
                </motion.div>
              ))}
            </div>
          </div>
