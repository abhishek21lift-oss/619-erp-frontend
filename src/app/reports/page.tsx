...
function ReportsContent() {
  ...
  useEffect(() => {
    setLoading(true);
    // Stable indexes regardless of role: [monthly, dues, trainers]
    Promise.all([
              ...
              {tab === 'monthly' && (
                <>
                  ...
                  <div className="card mb-3">
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '.4rem', height: 180 }}>
                      {fullYear.map((m, i) => (
                        <div
                          key={i}
                          title={`${m.month}: ${fmt(m.revenue)} (${m.count} payments)`}
                          style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', gap: 4 }}
                        >
                          <div style={{ fontSize: 10, color: 'var(--muted)', marginTop: 'auto', whiteSpace: 'nowrap', fontWeight: 600 }}>
                          ...
                          <div style={{
                            ...
                            boxShadow: m.revenue > 0 ? '0 0 18px var(--brand-glow)' : 'none',
