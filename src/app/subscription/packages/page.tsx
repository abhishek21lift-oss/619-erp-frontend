...
const inp = { width: '100%', padding: '11px 14px', borderRadius: 10, border: '1px solid #d1d5db', background: '#fff', color: '#111827', fontSize: 13, outline: 'none' } as const;
      ...
      {open && (
        <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <m.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
            ...
            style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 20, padding: '32px 36px', maxWidth: 380, width: '90%', textAlign: 'center', boxShadow: '0 16px 60px rgba(0,0,0,0.12)' }}>
            ...
            <h3 style={{ fontSize: 18, fontWeight: 700, color: '#111827', margin: '0 0 8px' }}>Delete?</h3>
            <p style={{ fontSize: 13, color: '#6b7280', margin: '0 0 24px' }}>This action cannot be undone.</p>
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={onCancel}
                style={{ flex: 1, padding: '10px', borderRadius: 10, border: '1px solid #d1d5db', background: '#f9fafb', color: '#374151', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                Cancel
              </button>
...
function PackagesTab() {
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: '1 1 240px', maxWidth: 320 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search packages..."
            style={{ width: '100%', padding: '10px 12px 10px 36px', borderRadius: 12, border: '1px solid #d1d5db', background: '#fff', color: '#111827', fontSize: 13, outline: 'none' }} />
        </div>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '60px 0', color: '#6b7280', fontSize: 14 }}>Loading packages...</div>
      ) : filtered.length === 0 ? (
        <m.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          style={{ textAlign: 'center', padding: '60px 24px', borderRadius: 20, background: '#fff', border: '1px solid rgba(0,0,0,0.07)' }}>
          <Package size={48} color={a} style={{ margin: '0 auto 12px', display: 'block' }} />
          <h3 style={{ fontSize: 17, fontWeight: 700, color: '#111827', margin: 0 }}>No Packages Yet</h3>
          <p style={{ fontSize: 13, color: '#6b7280', margin: '6px 0 20px' }}>Create your first PT package to get started.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
          {filtered.map((pkg, i) => (
            <m.div key={pkg.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              style={{ borderRadius: 18, background: '#fff', borderLeft: `3px solid ${a}`, borderTop: '1px solid rgba(0,0,0,0.07)', borderRight: '1px solid rgba(0,0,0,0.07)', borderBottom: '1px solid rgba(0,0,0,0.07)', padding: 20, position: 'relative' }}>
              <div style={{ position: 'absolute', top: 16, right: 16, display: 'flex', gap: 4 }}>
                <button onClick={() => openEdit(pkg)} style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: '#f3f4f6', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: a }}>
                  <Pencil size={13} />
                </button>
                <button onClick={() => setDeleteId(pkg.id)} style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: '#f3f4f6', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#dc2626' }}>
                  <Trash2 size={13} />
                </button>
              </div>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: '#111827', margin: '0 0 4px', paddingRight: 76 }}>{pkg.name}</h3>
              {pkg.goal_type && (
              ...
              {pkg.description && <p style={{ fontSize: 12, color: '#6b7280', margin: '8px 0 0', lineHeight: 1.4 }}>{pkg.description}</p>}
              <div style={{ fontSize: 28, fontWeight: 800, color: a, letterSpacing: '-0.03em', margin: '10px 0 2px' }}>{fmtINR(pkg.price)}</div>
              <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 14 }}>{pkg.session_count} sessions · {pkg.duration_days} days</div>
        ...
        {showForm && (
          <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <m.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }}
              style={{ width: '100%', maxWidth: 520, borderRadius: 22, background: '#fff', border: `1px solid rgba(0,0,0,0.07)`, padding: 28, boxShadow: '0 24px 80px rgba(0,0,0,0.12)' }}
              onClick={e => e.stopPropagation()}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: '#111827', margin: 0 }}>{editing ? 'Edit Package' : 'Create Package'}</h3>
                <button onClick={() => setShowForm(false)} style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: '#f3f4f6', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}>
                  <X size={15} />
                </button>
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 24 }}>
                <button onClick={() => setShowForm(false)} style={{ padding: '10px 18px', borderRadius: 10, border: '1px solid #d1d5db', background: '#f9fafb', color: '#374151', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
...
function PlansTab() {
  ...
  return (
    <>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '60px 0', color: '#6b7280', fontSize: 14 }}>Loading plans...</div>
      ) : filtered.length === 0 ? (
        <m.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          style={{ textAlign: 'center', padding: '60px 24px', borderRadius: 20, background: '#fff', border: '1px solid rgba(0,0,0,0.07)' }}>
          <FileText size={48} color={a} style={{ margin: '0 auto 12px', display: 'block' }} />
          <h3 style={{ fontSize: 17, fontWeight: 700, color: '#111827', margin: 0 }}>No Plans Yet</h3>
          <p style={{ fontSize: 13, color: '#6b7280', margin: '6px 0 20px' }}>Create your first subscription plan to get started.</p>
          <m.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={openCreate} style={{ ...btnPrim, display: 'inline-flex' }}>
            <Plus size={15} /> Create Plan
          </m.button>
        </m.div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
          {filtered.map((plan, i) => (
            <m.div key={plan.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              style={{ borderRadius: 18, background: '#fff', borderLeft: `3px solid ${a}`, borderTop: '1px solid rgba(0,0,0,0.07)', borderRight: '1px solid rgba(0,0,0,0.07)', borderBottom: '1px solid rgba(0,0,0,0.07)', padding: 20, position: 'relative' }}>
              <div style={{ position: 'absolute', top: 16, right: 16, display: 'flex', gap: 4 }}>
                <button onClick={() => openEdit(plan)} style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: '#f3f4f6', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: a }}>
                  <Pencil size={13} />
                </button>
                <button onClick={() => setDeleteId(plan.id)} style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: '#f3f4f6', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#dc2626' }}>
                  <Trash2 size={13} />
                </button>
              </div>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: '#111827', margin: '0 0 4px', paddingRight: 76 }}>{plan.name}</h3>
              {plan.description && <p style={{ fontSize: 12, color: '#6b7280', margin: '4px 0 0', lineHeight: 1.4 }}>{plan.description}</p>}
      ...
      <AnimatePresence>
        {showForm && (
          <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            onClick={() => setShowForm(false)}>
            <m.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }}
              style={{ width: '100%', maxWidth: 460, borderRadius: 22, background: '#fff', border: `1px solid rgba(0,0,0,0.07)`, padding: 28, boxShadow: '0 24px 80px rgba(0,0,0,0.12)' }}
              onClick={e => e.stopPropagation()}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: '#111827', margin: 0 }}>{editing ? 'Edit Plan' : 'Create Plan'}</h3>
                <button onClick={() => setShowForm(false)} style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: '#f3f4f6', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}>
                  <X size={15} />
                </button>
              </div>
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px', position: 'relative' }}>
        ...
        <m.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
          ...
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 800, color: '#111827', letterSpacing: '-0.03em', margin: 0 }}>Plans & Packages</h1>
        ...
        <m.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
          style={{ display: 'flex', gap: 8, marginBottom: 28, background: '#f3f4f6', borderRadius: 14, padding: 4, width: 'fit-content', border: '1px solid rgba(0,0,0,0.07)' }}>
          {TABS.map(t => (
