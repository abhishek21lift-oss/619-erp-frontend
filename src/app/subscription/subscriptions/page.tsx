      ...
      <m.div variants={cv} initial="hidden" animate="visible" style={{ position: 'relative', zIndex: 2 }}>
        <m.div variants={iv} style={{ position: 'relative', overflow: 'hidden', borderRadius: 22, padding: '40px 36px', marginBottom: 28, background: 'linear-gradient(135deg, #fffbeb, #fef3c7, #fff)', border: '1px solid rgba(0,0,0,0.07)', boxShadow: '0 2px 16px rgba(0,0,0,0.06)' }}>
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', gap: 20 }}>
            ...
            <div>
              <h1 style={{ fontSize: 26, fontWeight: 860, letterSpacing: '-0.03em', color: '#111827', margin: 0 }}>Subscriptions</h1>
              <p style={{ fontSize: 13, fontWeight: 500, color: '#6b7280', margin: '4px 0 0 0', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <m.div variants={iv} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12, marginBottom: 20, padding: '14px 18px', borderRadius: 16, background: '#fff', border: '1px solid rgba(0,0,0,0.07)', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
          <div style={{ position: 'relative', flex: '1 1 220px', maxWidth: 340 }}>
            ...
            <input type="text" placeholder="Search client or plan..." value={search} onChange={e => setSearch(e.target.value)}
              ...
              style={{ width: '100%', padding: '9px 12px 9px 34px', borderRadius: 11, fontSize: 12.5, fontWeight: 500, outline: 'none', border: '1.5px solid #d1d5db', background: '#fff', color: '#111827', transition: 'all 0.2s' }} />
          </div>
          <div style={{ display: 'flex', gap: 4, padding: 4, borderRadius: 11, background: '#f3f4f6', border: '1px solid rgba(0,0,0,0.06)' }}>
            {tabs.map(t => (
          ...
          <button onClick={fetchData} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 9, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: '1px solid rgba(0,0,0,0.07)', background: '#f9fafb', color: '#374151' }}>
                ...
                <m.div key={sub.id} variants={iv} whileHover={{ y: -4, boxShadow: '0 8px 24px rgba(0,0,0,0.1)' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        <p style={{ margin: 0, fontSize: 14, fontWeight: 720, color: '#111827', lineHeight: 1.3 }}>{sub.clientName}</p>
                        <p style={{ margin: '2px 0 0 0', fontSize: 11, fontWeight: 600, color: '#9ca3af' }}>{sub.planName}</p>
                      </div>
                    </div>
