...
const FALLBACK_PLANS: WorkoutPlan[] = [
      ...
      {[1, 2, 3].map((i) => (
        <div key={i} className="rounded-[20px] p-5 animate-pulse"
          ...
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-[10px]" style={{ background: 'rgba(15,23,42,0.06)' }} />
            <div>
              <div className="h-3.5 w-28 rounded" style={{ background: 'rgba(15,23,42,0.06)' }} />
              <div className="h-2.5 w-36 mt-1 rounded" style={{ background: 'rgba(15,23,42,0.04)' }} />
            </div>
          </div>
          <div className="mt-4 flex gap-2">
            <div className="h-5 w-16 rounded-full" style={{ background: 'rgba(15,23,42,0.06)' }} />
            <div className="h-5 w-20 rounded-full" style={{ background: 'rgba(15,23,42,0.06)' }} />
          </div>
          <div className="mt-4 h-1.5 w-full rounded-full" style={{ background: 'rgba(15,23,42,0.06)' }} />
        </div>
      ))}
    </div>
  );
...
function WorkoutPlansContent() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setExercises(
        Array.isArray(exercisesRes) && exercisesRes.length > 0
      ...
      setWorkoutPlans(FALLBACK_PLANS);
      setClients(FALLBACK_CLIENTS);
    } finally {
