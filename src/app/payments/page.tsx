...
import {
  ...
  Sparkles, X, ChevronDown, Calendar,
  Receipt, Users, Target, Zap,
  Clock, AlertCircle,
  IndianRupee, Send,
} from 'lucide-react';
...
function MethodPill({ method }: { method: string }) {
  const cfg = METHOD_CONFIG[method] ?? { label: method, color: 'var(--text-muted)', bg: 'rgba(100,116,139,0.10)', border: 'rgba(100,116,139,0.25)', icon: null };
  return (
    <span style={{
...
export default function RevenueIntelligencePage() {
  ...
  const loadPayments = useCallback(async (silent = false) => {
    ...
    try {
      const data = await api.payments.list({ from: dateFrom, to: dateTo });
  ...
  async function handleDelete(id: string | number) {
    try {
      await http('/api/payments/' + id, { method: 'DELETE' });
                ...
                <form onSubmit={handleSubmit} style={{ padding: '28px 28px 40px' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: fSubmitting ? 'none' : '0 4px 16px rgba(124,58,237,0.35)',
                      transition: 'all 160ms ease',
                    }}
                  >
                    {fSubmitting ? (
