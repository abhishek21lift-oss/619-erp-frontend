        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <KpiCard label="Today's Revenue" value={fmtCompact(todayRev)} delta="+12.4%" deltaUp icon={<IndianRupee size={16} />} accentColor="#10b981" sparkData={sparkSeeds.today} sparkColor="#10b981" loading={loading} sub={`${todayPayments.length} transactions`} />
              <KpiCard label="Period Total" value={fmtCompact(totalRev)} delta="+8.2%" deltaUp icon={<TrendingUp size={16} />} accentColor="#7c3aed" sparkData={sparkSeeds.total} sparkColor="#7c3aed" loading={loading} sub={`${filtered.length} payments`} />
              <KpiCard label="Transactions" value={String(todayPayments.length)} delta="+3" deltaUp icon={<Receipt size={16} />} accentColor="#3b82f6" sparkData={sparkSeeds.txns} sparkColor="#3b82f6" loading={loading} sub="today" />
              <KpiCard label="Avg. Ticket" value={todayPayments.length > 0 ? fmtCompact(Math.round(todayRev / todayPayments.length)) : '—'} icon={<Target size={16} />} accentColor="#f59e0b" loading={loading} />
              <KpiCard label="Top Method" value={Object.entries(methodBreakdown).sort(([,a],[,b]) => b - a)[0]?.[0]?.replace('_',' ') ?? '—'} icon={<Zap size={16} />} accentColor="#ec4899" loading={loading} />
              <KpiCard label="Members Paid" value={String(new Set(todayPayments.map(p => p.client_id)).size)} icon={<Users size={16} />} accentColor="#06b6d4" loading={loading} sub="unique today" />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 16, marginBottom: 24 }}>
              ...
              <div style={{
                ...
                <SectionHeader
                  ...
                  action={
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: 20, padding: '3px 10px' }}>
                ...
                />
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <InsightCard icon={<TrendingUp size={15} />} title="Weekly Revenue Trend" value="Revenue up 18% vs last week" color="#10b981" sub="Strongest day: Thursday" />
                  <InsightCard icon={<Users size={15} />} title="Renewal Prediction" value="12 renewals likely tomorrow" color="#3b82f6" sub="Based on expiry patterns" />
                  <InsightCard icon={<Target size={15} />} title="PT Conversion" value="PT sales conversion +9%" color="#7c3aed" sub="vs last 30 days" />
                  <InsightCard icon={<AlertCircle size={15} />} title="Pending Dues Alert" value="Pending dues increased 9%" color="#f59e0b" sub="₹18,400 outstanding" />
                  <InsightCard icon={<Clock size={15} />} title="Peak Revenue Time" value="6–8 PM highest conversion" color="#06b6d4" sub="Avg ₹4,200 per hour" />
                </div>
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, overflow: 'hidden', marginBottom: 24 }}>
              <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                <div>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '7px 12px', minWidth: 200 }}>
                    <Search size={13} style={{ color: 'rgba(255,255,255,0.3)', flexShrink: 0 }} />
                    <input
                      placeholder="Search member or receipt…"
                      value={search}
                      onChange={e => { setSearch(e.target.value); setPage(1); }}
                      style={{ background: 'none', border: 'none', outline: 'none', color: 'rgba(255,255,255,0.75)', fontSize: 12, width: '100%' }}
                    />
                    {search && (
                  ...
                  <thead>
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      {['Member', 'Amount', 'Method', 'Date', 'Receipt', 'Notes', ''].map((h, i) => (
                        <th key={i} style={{ padding: '10px 18px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.7px', whiteSpace: 'nowrap' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {loading ? (
                        ...
                        <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                          {[180, 90, 90, 100, 100, 160, 40].map((w, j) => (
                            <td key={j} style={{ padding: '14px 18px' }}>
                              <div style={{ height: 12, width: w, borderRadius: 6, background: 'linear-gradient(90deg, rgba(255,255,255,0.04) 25%, rgba(255,255,255,0.08) 50%, rgba(255,255,255,0.04) 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s ease-in-out infinite' }} />
                            </td>
                          ))}
                        </tr>
                      ))
                      ...
                      <tr>
                        <td colSpan={7}>
                          <div style={{ padding: '60px 24px', textAlign: 'center' }}>
                            <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', color: 'rgba(255,255,255,0.2)' }}>
                              <Receipt size={20} />
                            </div>
                      ...
                      paginated.map((p) => (
                        <tr key={p.id} className="tx-row" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', transition: 'background 150ms ease' }}>
                          <td style={{ padding: '12px 18px', whiteSpace: 'nowrap' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              <div style={{ width: 32, height: 32, borderRadius: '50%', background: avatarColor(p.client_name), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: 'white', flexShrink: 0 }}>
                                {avatarInitials(p.client_name)}
                              </div>
                              <div>
                                <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.82)' }}>{p.client_name ?? '—'}</div>
                                {p.trainer_name && (<div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginTop: 1 }}>Trainer: {p.trainer_name}</div>)}
                              </div>
                            </div>
                          </td>
                          <td style={{ padding: '12px 18px', whiteSpace: 'nowrap' }}>
                            <span style={{ fontSize: 14, fontWeight: 800, color: '#10b981', fontVariantNumeric: 'tabular-nums' }}>{fmtAmount(Number(p.amount))}</span>
                          </td>
                          <td style={{ padding: '12px 18px' }}><MethodPill method={p.method} /></td>
                          <td style={{ padding: '12px 18px', whiteSpace: 'nowrap' }}>
                            <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', fontWeight: 500 }}>{fmtDate(p.date)}</span>
                          </td>
                          <td style={{ padding: '12px 18px' }}>
                            <span style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.35)', fontFamily: 'monospace', background: 'rgba(255,255,255,0.04)', borderRadius: 6, padding: '2px 7px' }}>{p.receipt_no ?? '—'}</span>
                          </td>
                          <td style={{ padding: '12px 18px', maxWidth: 180 }}>
                            <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>{p.notes ?? '—'}</span>
                          </td>
                          <td style={{ padding: '12px 18px' }}>
                            {isAdmin && (
                              deleteId === p.id ? (
                                <div style={{ display: 'flex', gap: 4 }}>
                                  <button onClick={() => handleDelete(p.id)} style={{ fontSize: 10, fontWeight: 700, color: '#f43f5e', background: 'rgba(244,63,94,0.1)', border: '1px solid rgba(244,63,94,0.25)', borderRadius: 6, padding: '3px 8px', cursor: 'pointer' }}>Confirm</button>
                                  <button onClick={() => setDeleteId(null)} style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.4)', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 6, padding: '3px 8px', cursor: 'pointer' }}>Cancel</button>
                                </div>
                              ) : (
                                <button
                                  onClick={() => setDeleteId(p.id)}
                                  style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(244,63,94,0.06)', border: '1px solid rgba(244,63,94,0.15)', color: 'rgba(244,63,94,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 150ms ease' }}
              ...
              {totalPages > 1 && (
                <div style={{ padding: '14px 24px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="btn-ghost" style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: page === 1 ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.6)', fontSize: 12, fontWeight: 600, cursor: page === 1 ? 'not-allowed' : 'pointer' }}>
                      <ChevronDown size={13} style={{ transform: 'rotate(90deg)' }} /> Prev
                    </button>
                    <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="btn-ghost" style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: page === totalPages ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.6)', fontSize: 12, fontWeight: 600, cursor: page === totalPages ? 'not-allowed' : 'pointer' }}>
          ...
          {showPanel && (
            <>
              <div onClick={() => setShowPanel(false)} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }} />
              <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 'min(480px, 100vw)', zIndex: 101, background: 'linear-gradient(180deg, #13131f 0%, #0f0f1a 100%)', borderLeft: '1px solid rgba(255,255,255,0.08)', boxShadow: '-24px 0 80px rgba(0,0,0,0.5)', overflowY: 'auto', animation: 'slideInRight 260ms cubic-bezier(0.34,1.56,0.64,1)' }}>
                <div style={{ padding: '24px 28px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', position: 'sticky', top: 0, background: 'rgba(19,19,31,0.95)', backdropFilter: 'blur(16px)', zIndex: 10 }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(124,58,237,0.15)', border: '1px solid rgba(167,139,250,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a78bfa' }}>
                        <Plus size={16} />
                      </div>
                  ...
                  <button onClick={() => setShowPanel(false)} style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.5)', cursor: 'pointer', transition: 'all 150ms ease' }} onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.1)'; }} onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.06)'; }}>
                    <X size={15} />
                  </button>
                </div>
                ...
                <form onSubmit={handleSubmit} style={{ padding: '28px 28px 40px' }}>
                  <div style={{ marginBottom: 20 }}>
                    <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', letterSpacing: '0.7px', marginBottom: 8 }}>Member *</label>
                    {selectedClient ? (
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: 12, padding: '12px 14px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 32, height: 32, borderRadius: '50%', background: avatarColor(selectedClient.name), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: 'white' }}>{avatarInitials(selectedClient.name)}</div>
                          <div>
                            <div style={{ fontSize: 14, fontWeight: 700, color: 'rgba(255,255,255,0.88)' }}>{selectedClient.name}</div>
                            {(selectedClient.balance_due ?? selectedClient.balance_amount ?? 0) > 0 && (<div style={{ fontSize: 11, color: '#f59e0b' }}>Due: {fmtAmount(selectedClient.balance_due ?? selectedClient.balance_amount ?? 0)}</div>)}
                          </div>
                        </div>
                        <button type="button" onClick={() => { setFClientId(''); setClientSearch(''); }} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.35)', cursor: 'pointer', display: 'flex' }}><X size={14} /></button>
                      </div>
                    ) : (
                      <div style={{ position: 'relative' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '11px 14px', transition: 'border-color 160ms ease' }}>
                          <Search size={14} style={{ color: 'rgba(255,255,255,0.3)', flexShrink: 0 }} />
                          <input placeholder="Search member…" value={clientSearch} onChange={e => setClientSearch(e.target.value)} style={{ background: 'none', border: 'none', outline: 'none', color: 'rgba(255,255,255,0.8)', fontSize: 13, width: '100%' }} />
                        </div>
                        {clientSearch && filteredClients.length > 0 && (
                          <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 20, background: '#1a1a2e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, overflow: 'hidden', marginTop: 4, boxShadow: '0 12px 32px rgba(0,0,0,0.4)' }}>
                            {filteredClients.map(c => (
                              <button key={c.id} type="button" onClick={() => { setFClientId(String(c.id)); setClientSearch(''); }} style={{ width: '100%', textAlign: 'left', padding: '10px 14px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 120ms' }} onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.04)'; }} onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'none'; }}>
                                <div style={{ width: 28, height: 28, borderRadius: '50%', background: avatarColor(c.name), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: 'white', flexShrink: 0 }}>{avatarInitials(c.name)}</div>
                                <div>
                                  <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.85)' }}>{c.name}</div>
                                  {(c.balance_due ?? c.balance_amount ?? 0) > 0 && (<div style={{ fontSize: 10, color: '#f59e0b' }}>Due: {fmtAmount(c.balance_due ?? c.balance_amount ?? 0)}</div>)}
                                </div>
                              </button>
                            ))}
                          </div>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, overflow: 'hidden', transition: 'border-color 160ms' }} onFocusCapture={e => { (e.currentTarget as HTMLDivElement).style.borderColor = 'rgba(167,139,250,0.4)'; }} onBlurCapture={e => { (e.currentTarget as HTMLDivElement).style.borderColor = 'rgba(255,255,255,0.1)'; }}>
                      <span style={{ padding: '12px 14px', fontSize: 15, fontWeight: 700, color: '#10b981', borderRight: '1px solid rgba(255,255,255,0.06)' }}>₹</span>
                      <input type="number" min="1" step="1" placeholder="0" value={fAmount} onChange={e => setFAmount(e.target.value)} required style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: 'rgba(255,255,255,0.88)', fontSize: 15, fontWeight: 700, padding: '12px 14px', fontVariantNumeric: 'tabular-nums' }} />
                      {Number(fAmount) > 0 && (<span style={{ padding: '0 14px', fontSize: 12, color: 'rgba(255,255,255,0.3)', fontWeight: 600 }}>{fmtCompact(Number(fAmount))}</span>)}
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                      {(['CASH', 'UPI', 'CARD', 'BANK_TRANSFER'] as const).map(m => {
                        ...
                        return (
                          <button key={m} type="button" onClick={() => setFMethod(m)} style={{ padding: '10px 8px', borderRadius: 10, background: active ? `${cfg.color}18` : 'rgba(255,255,255,0.03)', border: active ? `1.5px solid ${cfg.color}40` : '1px solid rgba(255,255,255,0.08)', color: active ? cfg.color : 'rgba(255,255,255,0.45)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, cursor: 'pointer', transition: 'all 150ms ease', fontSize: 10, fontWeight: 700 }}>
                            {cfg.icon}{cfg.label}
                          </button>
                        );
                      })}
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '11px 14px' }} onFocusCapture={e => { (e.currentTarget as HTMLDivElement).style.borderColor = 'rgba(167,139,250,0.4)'; }} onBlurCapture={e => { (e.currentTarget as HTMLDivElement).style.borderColor = 'rgba(255,255,255,0.1)'; }}>
                      <Calendar size={14} style={{ color: 'rgba(255,255,255,0.3)', flexShrink: 0 }} />
                      <input type="date" value={fDate} onChange={e => setFDate(e.target.value)} required style={{ background: 'none', border: 'none', outline: 'none', color: 'rgba(255,255,255,0.8)', fontSize: 13, fontWeight: 600, width: '100%', cursor: 'pointer' }} />
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: 28 }}>
                    ...
                    <textarea placeholder="Optional notes…" value={fNotes} onChange={e => setFNotes(e.target.value)} rows={3} style={{ width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '11px 14px', color: 'rgba(255,255,255,0.75)', fontSize: 13, resize: 'vertical', outline: 'none', fontFamily: 'inherit', lineHeight: 1.5, transition: 'border-color 160ms', boxSizing: 'border-box' }} onFocus={e => { e.target.style.borderColor = 'rgba(167,139,250,0.4)'; }} onBlur={e => { e.target.style.borderColor = 'rgba(255,255,255,0.1)'; }} />
                  </div>

                  {fError && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(244,63,94,0.08)', border: '1px solid rgba(244,63,94,0.2)', borderRadius: 10, padding: '10px 14px', marginBottom: 20 }}>
                      <AlertCircle size={14} style={{ color: '#f43f5e', flexShrink: 0 }} />
                      <span style={{ fontSize: 12, color: '#f43f5e', fontWeight: 600 }}>{fError}</span>
                    </div>
                  )}

                  <button type="submit" disabled={fSubmitting} style={{ width: '100%', padding: '14px', borderRadius: 14, border: 'none', background: fSubmitting ? 'rgba(124,58,237,0.4)' : 'linear-gradient(135deg, #7c3aed 0%, #6d28d9 100%)', color: 'white', fontSize: 14, fontWeight: 700, cursor: fSubmitting ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: fSubmitting ? 'none' : '0 6px 24px rgba(124,58,237,0.4)', transition: 'all 180ms ease', letterSpacing: '-0.01em' }} onMouseEnter={e => { if (!fSubmitting) (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 8px 30px rgba(124,58,237,0.55)'; }} onMouseLeave={e => { if (!fSubmitting) (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 6px 24px rgba(124,58,237,0.4)'; }}>
                    {fSubmitting ? (<><RefreshCw size={14} style={{ animation: 'spin 1s linear infinite' }} /> Recording…</>) : (<><CheckCircle2 size={15} /> Record Payment</>)}
                  </button>
                </form>
              </div>
            </>
