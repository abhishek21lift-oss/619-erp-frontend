      ...
      <AppShell>
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50/80 px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          <nav className="flex items-center gap-1.5 text-xs text-slate-400 font-medium">
            ...
            <span className="text-slate-700">Studio Status</span>
          </nav>
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Studio Status</h1>
                {allOperational ? (
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white border border-slate-200 text-slate-600 text-sm font-medium hover:bg-slate-50 transition-all shadow-sm"
            >
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="space-y-2">
                {SYSTEMS.map(sys => {
                  ...
                  return (
                    <div key={sys.id} className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/80 p-4 shadow-sm flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-slate-800">{sys.label}</p>
                        <p className="text-xs text-slate-400 truncate">{sys.sublabel}</p>
                      </div>
                      {sys.value && (
                        <div className="text-right flex-shrink-0">
                          <p className="text-sm font-bold text-slate-700">{sys.value}</p>
                          <p className="text-xs text-slate-400">Last: {sys.lastCheck}</p>
                        </div>
                      )}
              ...
              {/* Equipment Status */}
              <h2 className="text-sm font-semibold text-slate-700 flex items-center gap-2 mt-2">
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                  ...
                  {EQUIPMENT.map(eq => (
                    <div key={eq.name} className="p-3 flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-400 capitalize">{eq.status.replace('-',' ')}</p>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="px-4 py-2 bg-slate-50 border-t border-slate-100 flex items-center gap-4">
                  {[{c:'bg-emerald-400',l:'Available'},{c:'bg-blue-400',l:'In Use'},{c:'bg-amber-400',l:'Maintenance'}].map(s=>(
                    <span key={s.l} className="flex items-center gap-1.5 text-xs text-slate-500">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/80 p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <p className="text-sm font-semibold text-slate-700">Live Occupancy</p>
                    <p className="text-xs text-slate-400">Floor capacity: {capacity}</p>
                  </div>
                  <Gauge className="w-5 h-5 text-slate-400" />
                </div>
                <div className="relative">
                  <div className="flex items-end gap-2 mb-2">
                    <span className="text-4xl font-bold text-slate-900">{occupancy}</span>
                    <span className="text-sm text-slate-400 mb-1">/ {capacity} members</span>
                  </div>
                  ...
                  <p className="text-xs text-slate-400 mt-1.5">{pct}% capacity utilization</p>
                </div>
              </div>
                    ...
                    <div key={tr.name} className="flex items-center gap-3">
                      <div className="relative">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center">
                          <span className="text-xs font-bold text-slate-600">{tr.name[0]}</span>
                        </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-slate-700">{tr.name}</p>
                      ...
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                        ...
                        tr.status === 'in-session' ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-500'
                      }`}>
                        {tr.status.replace('-',' ')}
                      </span>
                    </div>
