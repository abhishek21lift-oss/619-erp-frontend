...
// with premium animated client metrics cards replacing the old donut charts.
...
function HeroDecoration() {
  return (
    <svg aria-hidden="true" viewBox="0 0 320 160" fill="none"
      xmlns="http://www.w3.org/2000/svg" className="hero-decoration">
      <ellipse cx="200" cy="80" rx="120" ry="72" fill="rgba(185,28,28,0.13)" />
      <ellipse cx="200" cy="80" rx="88"  ry="52" fill="rgba(185,28,28,0.10)" />
      <path d="M160 130 C158 110 148 95 140 78 C135 65 130 52 134 38 C138 28 148 22 152 14"
      ...
      <path d="M175 130 C178 110 190 95 198 78 C204 65 210 52 206 38 C202 28 192 22 188 14"
        stroke="rgba(220,38,38,0.55)" strokeWidth="3" strokeLinecap="round" />
      ...
      <path d="M167 130 C162 112 158 96 162 82 C165 70 172 62 168 52 C172 58 178 68 176 80 C180 68 184 56 182 44 C188 54 190 68 186 80 C192 70 196 60 194 50 C200 62 200 76 196 88 C202 80 208 72 206 62 C212 74 210 90 202 100 C208 96 216 92 218 84 C218 96 210 110 198 118 C196 122 192 126 190 130Z"
        fill="rgba(220,38,38,0.22)" stroke="rgba(220,38,38,0.45)" strokeWidth="1.2" />
      <circle cx="168" cy="100" r="22" fill="rgba(185,28,28,0.70)" stroke="rgba(220,38,38,0.60)" strokeWidth="1.5" />
      <text x="168" y="105" textAnchor="middle" fill="#fff" fontSize="13" fontWeight="800"
        fontFamily="system-ui,sans-serif">619</text>
    </svg>
  );
}
  ...
  return (
    <div className="hero-logo-wrap" aria-hidden="true">
      <svg viewBox="0 0 64 72" fill="none" xmlns="http://www.w3.org/2000/svg" className="hero-logo-svg">
        <path d="M32 4 L60 16 L60 42 C60 56 44 68 32 72 C20 68 4 56 4 42 L4 16 Z"
          fill="#b91c1c" stroke="#dc2626" strokeWidth="1.5" />
        <path d="M32 10 L54 20 L54 42 C54 53 42 64 32 68 C22 64 10 53 10 42 L10 20 Z"
...
function PeriodPicker({ value, onChange }: { value: Period; onChange: (p: Period) => void }) {
  return (
    <div role="tablist" aria-label="Time range" className="hero619-period-picker">
      {PERIOD_TABS.map((t) => (
        <button key={t.id} role="tab" aria-selected={value === t.id}
          onClick={() => onChange(t.id)}
          className={cn('hero619-period-btn', value === t.id ? 'hero619-period-btn--active' : '')}>
          {t.label}
        </button>
      ))}
    </div>
    ...
    return (
      <div className="px-5 py-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="grid grid-cols-4 gap-2 border-b border-slate-100 py-3 last:border-b-0">
            {[1,2,3,4].map((j) => (
              <div key={j} className="h-3 animate-pulse rounded bg-slate-200/70" />
            ))}
          </div>
        ))}
      </div>
        ...
        <tbody>
          {rows.map((p) => (
            <tr key={p.id} className="border-t border-slate-100 transition hover:bg-slate-50/60">
              <td className="px-5 py-3 font-medium text-slate-900">{p.client_name || '—'}</td>
              <td className="px-5 py-3 font-semibold tabular-nums text-emerald-700">{fmtINR(p.amount)}</td>
              <td className="px-5 py-3">
                <Badge tone={statusTone(p.method)} dot>{(p.method ?? 'CASH').toUpperCase()}</Badge>
              </td>
              <td className="hidden px-5 py-3 text-slate-600 md:table-cell">{p.trainer_name || '—'}</td>
            ...
            {rows.map((t, i) => (
              <li key={t.id} className="flex items-center justify-between gap-4 px-5 py-3">
                ...
                <div className="shrink-0 text-right">
                  ...
                  <p className="text-xs text-slate-500">{t.active_clients ?? 0} active</p>
                </div>
              </li>
            ))}
          </ul>
...
function periodCopy(p: Period) {
  switch (p) {
    ...
    case '7d':   return 'Rolling 7 days';
    case '30d':  return 'Rolling 30 days';
    case '90d':  return 'Rolling 90 days';
  }
}
