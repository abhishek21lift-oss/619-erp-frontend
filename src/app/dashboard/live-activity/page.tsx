      ...
      <AppShell>
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50/80 px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          <nav className="flex items-center gap-1.5 text-xs text-slate-400 font-medium">
            ...
            <span className="text-slate-700">Live Activity</span>
          </nav>
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Live Activity</h1>
              ...
              <p className="text-sm text-slate-500">Real-time operations monitoring · 619 Fitness Studio</p>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-400 hidden sm:block">Synced {lastSync}</span>
              <button
                onClick={refresh}
                className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white border border-slate-200 text-slate-600 text-sm font-medium hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm"
              >
                <RefreshCw className={`w-3.5 h-3.5 transition-transform ${pulse ? 'animate-spin' : ''}`} />
                <span className="hidden sm:inline">Refresh</span>
              </button>
              <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white border border-slate-200 text-xs font-medium text-slate-600 shadow-sm">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{items.length} events</span>
              </div>
              ...
              {items.map((item, idx) => (
                ...
                >
                  ...
                  <div className="flex flex-col items-center pt-1">
                    ...
                    {idx < items.length - 1 && <div className="w-px flex-1 mt-2 min-h-[28px] bg-slate-100" />}
                  </div>
                  {/* Icon */}
                  <div className={`w-9 h-9 rounded-xl bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200 flex items-center justify-center flex-shrink-0 shadow-sm ${item.glow} group-hover:scale-105 transition-transform`}>
                  ...
                  <div className="text-right flex-shrink-0">
                    {item.amount && <p className="text-sm font-bold text-slate-800">{item.amount}</p>}
                    <p className="text-xs text-slate-400">{item.ago}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={s.label} className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/80 p-5 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
